...
int main(int argc, char *argv[])
{
    ...
    while (DoMore) {
        ...
        switch (MajorOp) {
        case 0: /* Help */
            printf("%s%s%s", "1: Translate  2: Alist  3: ExternalAlist\n",
...
static void
Op_2(void) /* Alist: New, Free, Fill, Print, Externalize, hton, ntoh */
{
    int MinorOp;
    ...
    while (TRUE) {
        ...
        switch (MinorOp) {
        case 0:
            printf("%s%s", "1: New  2: Free  3: Fill  4: Print\n",
        ...
        {
            int w;
            if ((w = AskSlot(AVEC, "Which Alist slot? ")) < 0)
                break;
        ...
        case 3: /* Fill Alist */
        {
            ...
            if (GetInputOutput(&myin, &myout) < 0)
                break;
            ...
            if (ThisAl->TotalNoOfEntries <
                ThisAl->PlusEntriesInUse + ThisAl->MinusEntriesInUse)
        ...
        case 5: /* Externalize */
        {
            ...
            if ((w = AskSlot(AVEC, "Which Alist? ")) < 0)
                break;
            if ((i = NewSlot(EVEC)) < 0)
                break;
            printf("Vec[%d][%d] allocated\n", EVEC, i);

            if (AL_Externalize(Vec[AVEC][w].Gptr.A, &(Vec[EVEC][i].Gptr.E)) !=
                0) {
...
static void Op_3(
    void) /* ExternalAlist:   New, Free, Fill, Print, Internalize, hton, ntoh */
{
    int MinorOp;
    ...
    while (TRUE) {
        ...
        case 2: /* Free External Alist */
        {
            ...
            if ((w = AskSlot(EVEC, "Which ExternalAlist? ")) < 0)
                break;
...
static void Op_4(void) /* InternalCPS: New, Free, Get, Print, hton, ntoh */
{
    ...
    while (TRUE) {
        ...
        case 5: /* hton */
        {
            ...
            if ((w = AskSlot(CVEC, "Which InternalCPS slot? ")) < 0)
                break;
...
static void Op_5(void) /* ExternalCPS:  New, Free, Get, Print, hton, ntoh */
{
    ...
    while (TRUE) {
        ...
        switch (MinorOp) {
        case 0:
            printf("%s%s", "1: New  2: Free  3: Get  4: Print\n", "5:  Quit\n");
            break;
        ...
        case 2: /* Free External CPS */
        {
            ...
            if ((w = AskSlot(ECVEC, "Which ExternalCPS? ")) < 0)
                break;
...
static void Op_6(void) /* Miscellaneous:  CheckRights */
{
    ...
    while (TRUE) {
        ...
        switch (MinorOp) {
        case 0:
            printf("%s", "1: CheckRights  2: IsAMember 3: Quit\n");
            break;

        case 1: /* Check Rights */
        {
            int w1, w2, r;
            if ((w1 = AskSlot(AVEC, "Which Alist? ")) < 0)
                break;
            if ((w2 = AskSlot(CVEC, "Which InternalCPS? ")) < 0)
                break;

            printf("AL_CheckRights() = %d\n",
                   AL_CheckRights(Vec[AVEC][w1].Gptr.A, Vec[CVEC][w2].Gptr.C,
                                  &r));
            printf("rights = 0x%x\n", r);
        } break;
...
static int NewSlot(IN int VecType)
{
    int i;
    for (i = 0; i < SLOTMAX; i++)
        if (Vec[VecType][i].Allocated == FALSE)
            break;
    if (i >= SLOTMAX) {
