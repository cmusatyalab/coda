...
char DefKey[RPC2_KEYSIZE] = { '\144', '\162', '\163', '\145',
                              '\165', '\163', '\163', '\040' };
...
RPC2_EncryptionKey *PWArray =
    NULL; /* pointer to array of passwords indexed by ViceId */
...
long PWGetKeys(RPC2_CountedBS *cIdent, RPC2_EncryptionKey hKey,
               RPC2_EncryptionKey sKey)
{
    ...
    if (!IsAUser(vid) || IsADeletedUser(vid))
        return (-1);
...
static void EnlargePW(int newSize)
{
    ...
    LogMsg(0, AuthDebugLevel, stdout, "Reallocing from %d to %d", PWLen,
           newSize);
    PWArray =
        (RPC2_EncryptionKey *)realloc((char *)PWArray, newSize * RPC2_KEYSIZE);
    CODA_ASSERT(PWArray != NULL);
    memset(PWArray[PWLen], 0, (newSize - PWLen) * RPC2_KEYSIZE);
    PWLen = newSize;
}

static void AppendPW(int vId, RPC2_EncryptionKey eKey, char *otherInfo,
                     int agentId)
{
...
long PWNewUser(RPC2_Handle cid, RPC2_Integer viceId, RPC2_EncryptionKey initKey,
               RPC2_String otherInfo)
{
    ...
    if (!IsAdministrator(u.p)) {
        ...
        LogMsg(-1, 0, stdout, "AuthNewUser() attempt on  %s by %s denied",
               GetVname(viceId, buf1), GetVname(u.p->ViceId, buf2));
        return (AUTH_DENIED);
    }
...
long PWChangeUser(RPC2_Handle cid, RPC2_Integer viceId,
                  RPC2_EncryptionKey newKey, RPC2_String otherInfo)
{
    ...
    if (!IsAdministrator(u.p)) {
        char buf1[PRS_MAXNAMELEN], buf2[PRS_MAXNAMELEN];
        LogMsg(-1, 0, stdout, "AuthChangeUser() attempt on  %s by %s denied",
               GetVname(viceId, buf1), GetVname(u.p->ViceId, buf2));
        return (AUTH_DENIED);
    }
...
long PWChangePasswd(RPC2_Handle cid, RPC2_Integer viceId, RPC2_String Passwd)
{
    ...
    if (viceId != u.p->ViceId && !IsAdministrator(u.p)) {
        char buf1[PRS_MAXNAMELEN], buf2[PRS_MAXNAMELEN];
        LogMsg(-1, 0, stdout, "AuthChangePassd() attempt on %s by %s denied",
               GetVname(viceId, buf1), GetVname(u.p->ViceId, buf2));
        return (AUTH_DENIED);
    }

    if (!IsAUser(viceId) || IsADeletedUser(viceId))
        return (AUTH_FAILED);

    /* Make the change */
    if (BogusKey(newPasswd))
        return (AUTH_BADKEY);
