...
static int dir_DirEntry2VDirent(PDirEntry ep, struct venus_dirent *vd,
                                VolumeId vol, RealmId realm)
{
    ViceFid vfid;
    struct CodaFid kfid;
...
int DIR_Delete(struct DirHeader *dir, const char *entry)
{
    ...
    firstitem =
        dir_FindItem(dir, entry, &preventry, &index, CLU_CASE_SENSITIVE);
    if (!firstitem)
        return ENOENT;
...
int DIR_PrintEntry(PDirEntry entry, FILE *f)
{
    ...
    fprintf(f, "next: %hu, flag %d fid: (%x.%x) %s\n", ntohs(entry->next),
            entry->flag, fid.Vnode, fid.Unique, entry->name);
    ...
    for (i = 0; i < DIR_MAXPAGES; i++) {
        if (dir->dirh_allomap[i] != EPP) {
            ...
            fprintf(f, "page %d, tag %d, freecount %d, set %d, bitmap: \n", i,
                    (int)ntohl(ph->tag), freecount, setbits);
            fprintf(f, "%s\n\n", bitmap);
        }
    }
...
static void fid_NFidV2Fid(struct DirNFid *dnfid, VolumeId vol,
                          struct ViceFid *fid)
{
...
int DIR_Convert(PDirHeader dir, char *file, VolumeId vol, RealmId realm)
{
    ...
    for (i = 0; i < NHASH; i++) {
        ...
        while (num != 0) {
            ...
            direntlen = dir_DirEntry2VDirent(
...
struct DirEntry *dir_GetBlob(struct DirHeader *dir, long blobno)
{
    ...
    return (struct DirEntry *)((char *)dir + blobno * sizeof(struct DirBlob));
}
                                     ...
                                     int flags)
{
    ...
    default:
        fprintf(
            stdout,
            "!!! You might have an unsupported Coda kernel module. Please update coda.o !!!\n");
        CODA_ASSERT(0);
    }
	...
	*/
    ...
    for (i = pages; i < DIR_MAXPAGES; i++) {
        if (pdh->dirh_allomap[i] != EPP) {
            printf(
                "Page %d is partially empty but appears after directory ended",
                i);
            return 0;
        }
    }
	...
	*/
    for (i = 0; i < NHASH; i++) {
        ...
        while (1) {
            ...
            if (j != i) {
                printf(
                    "Dir entry %p, name %s, should be i nhash"
    ...
    for (i = 0; i < pages; i++) {
        ...
        for (j = 0; j < EPP / 8; j++) {
            if (eaMap[fcount + j] != ph->freebitmap[j]) {
                printf(
                    "blob bitmap error: page %d, map offset %d,"
                    "%x should be %x.\n",
                    i, j, ph->freebitmap[j], eaMap[fcount + j]);
                return 0;
            }
        }
    }
