...
int RepairRename(int nreplicas, resreplica *dirs, resdir_entry **deGroup,
                 int nDirEntries, listhdr **ops, VolumeId RepVolume,
                 char *realm)
{
    ...
    for (i = 0; i < nreplicas; i++) {
        ...
        if (!GetParent(realm, &tmpfid, &parentfid[i], parentpath[i],
                       childpath[i])) {
            nobjfound++;
            usepath[i] = 1;
        }
    ...
    if (nreplicas != nobjfound) {
        printf(
            "***************************************************************\n");
        printf(
            "Warning:  Object was renamed at some sites and removed at other\n");
        printf(
            "If you decide to preserve the object, you might need to perform\n");
        printf(
            "another compare/doRepair cycle after repairing  the directory with the");
    ...
    for (i = 0; i < nreplicas; i++) {
        ...
        if (!usepath[i])
            continue;
        snprintf(curpath, sizeof(curpath), "%s/%s", parentpath[i],
                 childpath[i]);
    ...
    if (i == nreplicas) {
        ...
        for (int j = 0; j < nreplicas; j++)
            if ((parentfid[j].Vnode == dirs[j].fid.Vnode) &&
                (parentfid[j].Unique == dirs[j].fid.Unique) && usepath[j]) {
                if (isDir)
        ...
        for (int j = 0; j < nreplicas; j++) {
            ...
            if (IsCreatedEarlier(ops, j, parentfid[j].Vnode,
                                 parentfid[j].Unique) ||
                IsCreatedEarlier(ops, j, parentfid[i].Vnode,
                                 parentfid[i].Unique)) {
                printf("more repair actions are needed on replica %s ",
                       dirs[j].path);
                printf(
                    "please use the \"comparedir\" and \"dorepair\" commands again\n");
                continue;
            }
...
int RepairSubsetCreate(int nreplicas, resreplica *dirs, resdir_entry **deGroup,
                       int nDirEntries, listhdr **ops, VolumeId RepVolume)
{
    int isDir = ISDIRVNODE(deGroup[0]->fid.Vnode);
    struct repair rep;
    ...
    for (i = 0; i < nreplicas; i++) {
        if (!ObjFound[i]) {
            if (isDir)
                ...
                str = (char *)malloc(strlen(deGroup[0]->name) +
                                     strlen(dirs[deGroup[0]->index].path) + 1);
...
int RepairSubsetRemove(int nreplicas, resreplica *dirs, resdir_entry **deGroup,
