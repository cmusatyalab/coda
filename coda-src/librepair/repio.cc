...
int repair_getdfile(char *fname, int infd, int *replicaCount,
     ...
     */
{
    ...
    *replicaList =
        (struct listhdr *)calloc(*replicaCount, sizeof(struct listhdr));
    if (*replicaList == NULL) {
        sprintf(
            errmsg,
            "repair_getdfile: replicaList allocation failed!\n\telements= %x\tx= %d\tsizeof(struct listhdr)= %lu\n",
    ...
    for (i = 0; i < *replicaCount; i++) {
        if ((*replicaList)[i].repairCount > 0) {
            ...
            if (!r) {
                sprintf(
                    errmsg,
                    "repair_getdfile: repair calloc failed!\n\tcount = %d\tsize = %lu\n\n",
...
int repair_getdfile(char *fname, int *replicaCount,
                    struct listhdr **replicaList)
{
...
#define ADVANCE()                                       \
    ...
    if (!*c)
        return (-2); /* only place premature eof is ok */

    /* Parse opcode */
    NEXTFIELD();
    if (!strcmp(c, "createf")) {
        rs->opcode = REPAIR_CREATEF;
        goto Opfound;
    }
    if (!strcmp(c, "created")) {
        rs->opcode = REPAIR_CREATED;
        goto Opfound;
    }
    if (!strcmp(c, "creates")) {
        rs->opcode = REPAIR_CREATES;
        goto Opfound;
    }
    if (!strcmp(c, "createl")) {
        rs->opcode = REPAIR_CREATEL;
        goto Opfound;
    }
    if (!strcmp(c, "removefsl")) {
        rs->opcode = REPAIR_REMOVEFSL;
        goto Opfound;
    }
    if (!strcmp(c, "removed")) {
        rs->opcode = REPAIR_REMOVED;
        goto Opfound;
    }
    if (!strcmp(c, "setacl")) {
        rs->opcode = REPAIR_SETACL;
        goto Opfound;
    }
    if (!strcmp(c, "setnacl")) {
        rs->opcode = REPAIR_SETNACL;
        goto Opfound;
    }
    if (!strcmp(c, "setmode")) {
        rs->opcode = REPAIR_SETMODE;
        goto Opfound;
    }
    if (!strcmp(c, "setowner")) {
        rs->opcode = REPAIR_SETOWNER;
        goto Opfound;
    }
    if (!strcmp(c, "setmtime")) {
        rs->opcode = REPAIR_SETMTIME;
        goto Opfound;
    }
    if (!strcmp(c, "replica")) {
        rs->opcode = REPAIR_REPLICA;
        goto Opfound;
    }
    if (!strcmp(c, "mv")) {
        rs->opcode = REPAIR_RENAME;
        goto Opfound;
    }
...
Opfound:
    ...
    for (i = 0; i < REPAIR_MAX; i++) {
        ...
        default:
            CODA_ASSERT(0); /* better not be anything else! */
        }
