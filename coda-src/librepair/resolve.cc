...
static int getfid(char *path, ViceFid *Fid, ViceVersionVector *VV,
                  struct ViceIoctl *vi)
{
    char buf[2048];
    vi->out      = buf;
    ...
    if (pioctl(path, _VICEIOCTL(_VIOC_GETFID), vi, 0)) {
        ...
        sscanf(symval, "@%x.%x.%x", &(Fid->Volume), &(Fid->Vnode),
               &(Fid->Unique));
...
struct Acl *ParseAcl(char *buf)
{
    ...
    ta->pluslist =
        (struct AclEntry *)malloc(ta->nplus * sizeof(struct AclEntry));
    ta->minuslist =
        (struct AclEntry *)malloc(ta->nminus * sizeof(struct AclEntry));
...
int getunixdirreps(int nreplicas, char *names[], resreplica **reps)
{
    ...
    *reps = dirs = (resreplica *)calloc(nreplicas, sizeof(resreplica));
    ...
    CODA_ASSERT(direntriesarr = (resdir_entry *)malloc(direntriesarrsize *
                                                       sizeof(resdir_entry)));
    ...
    for (j = 0; j < nreplicas; j++) {
        ...
        if (res_getfid(names[j], &Fid, &VV))
            return -1;
        dirs[j].nentries = count;

        CODA_ASSERT(
            Fid.Volume !=
            0xffffffff); /* will trigger if we have an expanded dirent */
...
static int nextindex()
{
    ...
    if (totaldirentries >= direntriesarrsize) {
        direntriesarr = (resdir_entry *)realloc(direntriesarr,
...
int GetConflictType(int nreplicas, resreplica *dirs, resdir_entry **deGroup,
                    int nentries, int *conflictType, char *realm)
{
    int i;
...
int InsertListHdr(int nreplicas, struct repair *rep, struct listhdr **ops,
                  int index)
{
    int size;
    ...
    if (repList == 0)
        return -1;
    if (size > 0)
        memcpy(repList, (*ops)[index].repairList,
               (size * sizeof(struct repair)));
...
int InRepairList(struct listhdr *opList, unsigned opcode, VnodeId vnode,
                 Unique_t unique)
{
    struct repair *repList = opList->repairList;

    for (unsigned int i = 0; i < opList->repairCount; i++)
        if ((repList[i].opcode == opcode) && (repList[i].parms[1] == vnode) &&
            (repList[i].parms[2] == unique))
            return 1;
    return 0;
}

/* checks if a fid has been created by an earlier operation in a repair list */
int IsCreatedEarlier(struct listhdr **opList, int index, VnodeId vnode,
                     Unique_t unique)
{
    ...
    for (unsigned int i = 0; i < count; i++)
        if ((repList[i].opcode == REPAIR_CREATED) &&
            (repList[i].parms[1] == vnode) && (repList[i].parms[2] == unique))
            return 1;
    return 0;
}
                     ...
                     VolumeId RepVolume, char *realm)
{
    ...
    case ALL_PRESENT:
        break;
    case SUBSET_RENAME:
        RepairRename(nreplicas, dirs, deGroup, nentries, opList, RepVolume,
                     realm);
        break;
    case SUBSET_CREATE:
...
int NameNameResolve(int first, int last, int nreplicas, resreplica *dirs,
                    struct listhdr **opList, struct repinfo *inf)
{
    ...
    if (!lastslash)
        ...
        *lastslash = '\0';
        lastslash =
            strrchr(replicatedname, '/'); // the / before the replica name
        if (!lastslash)
            printf("Couldn't find the parent directory of %s\n",
                   sortedArrByName[first]->name);
        else {
    ...
    for (i = first; i < last; i++) {
        ...
        printf("%s%s\n\tFid: (%08x.%08x) VV: %s\n", dirs[rde->index].path,
               sortedArrByName[i]->name, rde->fid.Vnode, rde->fid.Unique, fid);
    }
    ...
    for (i = first; i < last; i++) {
		...
		*  -- keep file if link in fixed dir points to the replica */
            ...
            sprintf(fixedpath, "%s%s%s", inf->fixed,
                    ((inf->fixed[(strlen(inf->fixed) - 1)] == '/') ? "" : "/"),
                    rde->name);
            if ((lstat(fixedpath, &sbuf) < 0) || (!(S_ISLNK(sbuf.st_mode))))
                answers[i - first] =
                    1; /* remove anything not explicitly saved */
            else {
                memset(lnpath, 0, MAXPATHLEN);
                if ((readlink(fixedpath, lnpath, MAXPATHLEN - 1) <
                     0) /* keep data if link is bad */
                    || (strcmp(lnpath, nnpath) ==
                        0)) /* or if it points to the replica */
                    answers[i - first] = 0;
                else
                    answers[i - first] = 1;
            }
    ...
    if ((nno == nobjects) && (inf->interactive))
        printf(
            "WARNING: Nothing will change; NAME/NAME conflict will remain\n");
        ...
        for (i = 0; i < nobjects; i++) {
            if (!answers[i]) {
                if (goodvnode == (VnodeId)-1) {
                    ...
                    printf(
                        "Please try to rename or remove one of the two objects:\n");
               ...
               char *realm)
{
    ...
    sortedArrByName =
        (resdir_entry **)malloc(totaldirentries * sizeof(resdir_entry *));
    ...
    while (first < totaldirentries) {
        while ((next < totaldirentries) &&
               !(strcmp(sortedArrByName[next]->name,
                        sortedArrByName[first]->name)))
            next++;
    ...
    for (i = 0, j = 1; i < totaldirentries; i += j) {
        ...
        for (j = 1; (i + j) < totaldirentries; j++)
            if (resdirCompareByFidName(&(sortedArrByFidName[i]),
                                       &sortedArrByFidName[i + j]))
                break;
        if (sortedArrByFidName[i]->lookedAt)
            continue;

        rc = GetConflictType(nreplicas, dirs, &(sortedArrByFidName[i]), j,
                             &conflict, realm);
        if (rc) {
            if (inf->interactive)
            ...
            ResolveConflict(nreplicas, dirs, &(sortedArrByFidName[i]), j,
                            conflict, opList, RepVolume, realm);
    }
...
int GetParent(char *realm, ViceFid *cfid, ViceFid *dfid, char *dpath,
              char *childname)
{
