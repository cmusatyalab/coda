...
static int lkdb_GetFilenameFromSHA(struct lkdb *dbp,
                                   ...
                                   char *hitname, int hitnamelen, char *emsg,
                                   int emsglen);
...
*/
    ...
    list_for_each(p, lkdbchain)
    {
        ...
        if (*emsgbuf)
            return (0); /* fatal error in GetFilenameFromSHA() */
    }
...
FoundSHA:; /* found the sha in the lkdb pointed to by dbp */
    ...
    if (codaprefix && !strncmp(hitpathname, codaprefix, strlen(codaprefix))) {
        snprintf(emsgbuf, emsgbuflen,
                 "Avoiding recursive lookaside on hitpathname: %s\n",
                 hitpathname);
        return (0);
    }
    ...
    if (err < 0) {
        snprintf(emsgbuf, emsgbuflen, "CopyAndComputeSHA(%s): %s", hitpathname,
                 strerror(errno));
...
static int lkdb_BindDB(struct lkdb *dbp, char *dbpathname, char *emsgbuf,
                       int emsgbuflen)
{
  ...
  */
    ...
    if (emsgbuflen > 0)
        *emsgbuf = 0; /* null msg for success case */
    ...
    desc = malloc(dlen);
    if (!desc) {
        snprintf(emsgbuf, emsgbuflen, "%s can't allocate descriptor",
                 dbp->dbname);
        goto err;
    }
...
BadDescriptorRecord:
    snprintf(emsgbuf, emsgbuflen, "%s: bad descriptor record '%s'", dbp->dbname,
             desc);
    free(desc);
err:
    return (0);
...
static int lkdb_GetFilenameFromSHA(struct lkdb *dbp,
                                   unsigned char sha[SHA_DIGEST_LENGTH],
                                   char *hitpath, int hitpathlen, char *emsgbuf,
                                   int emsgbuflen)
{
...
*/
    ...
    dpos = rwcdb_datapos(&dbp->dbh);
    if (dlen < 2)
        return 0; /* we need at least one character and a '\0' */
...
int LKParseAndExecute(char *command, char *emsgbuf, int emsgbuflen)
{
   ...
   */
    ...
    char *emptr;
#define USAGESTR \
    "Usage: cfs lka [--clear] [+/-<db1>] [+/-<db2>] [+/-<db3>] ...\n        cfs lka --list\n"
    ...
    while (nextoffset < maxoffset) {
        /* get next command */
        if (sscanf(&command[nextoffset], " +%s%n", cmdarray[cmdcount].dbname,
                   &xo)) {
            cmdarray[cmdcount].op = '+';
        } else {
            if (sscanf(&command[nextoffset], " -%s%n",
                       cmdarray[cmdcount].dbname, &xo)) {
                cmdarray[cmdcount].op = '-';
            } else
                goto BogusCommand;
        }
        ...
        cmdarray = (struct cmd *)realloc(cmdarray,
                                         (1 + cmdcount) * sizeof(struct cmd));
        if (!cmdarray) {
        ...
        case '+':
            ...
            if (mylk && lkdb_BindDB(mylk, cmdarray[i].dbname, emptr, eleft)) {
                ...
                snprintf(emptr, eleft, "%s added (%d entries)\n",
                         cmdarray[i].dbname, mylk->entrycount);
                RESIZEBUF();
            } else {
                delete_lkdb(mylk);
...
static void ListAll(char *mptr, int mlen)
{
    ...
    list_for_each(p, lkdbchain)
    {
        ...
        if (bytesleft <= 0)
            break;
        snprintf(where, bytesleft,
                 "%s (%d entries) [attempts = %d  hits = %d  shafails = %d]\n",
                 dbp->dbname, dbp->entrycount, dbp->attempts, dbp->hits,
                 dbp->shafails);
