...
{
    LogMsg(1, SrvDebugLevel, stdout, "Entering CheckAndPerformRename\n");
    ...
    // check semantics for rename
    {
        ...
        if ((errorCode = CheckResolveRenameSemantics(
                 r, volptr, dFid, vlist, &sv, &tv, &sdv, &tdv, AllLogs, inclist,
                 newinclist, blocks)))
    ...
    if (!errorCode) {
        int tblocks = 0;
        CODA_ASSERT(CleanRenameTarget(r, vlist, volptr, VSGVolnum, AllLogs,
                                      &tblocks) == 0);
        *blocks += tblocks;
        tblocks = 0;
        PerformRename(NULL, VSGVolnum, volptr, sdv->vptr, tdv->vptr, sv->vptr,
                      (tv ? tv->vptr : 0), r->name1, r->name2,
    ...
    if (errorCode && errorCode == EINCONS) {
	   ...
	   * object. The question is, should we _always_ do this? */
            ...
            LogMsg(0, SrvDebugLevel, stdout,
                   ...
                   r->name1, r->u.mv.svnode, r->u.mv.sunique, r->name2,
                   r->u.mv.tvnode, r->u.mv.tunique, HintFid->Vnode,
                   HintFid->Unique);
    ...
    LogMsg(1, SrvDebugLevel, stdout, "ResolveRename returning %d", errorCode);
    return (errorCode);
}

// mark an object inconsistent and form and add the ilist entry
static void AddToIncList(dlist *inclist, dlist *vlist, Volume *volptr,
                         ViceFid *fid, int markobjinc)
{
    ...
    if (ov && ov->vptr) {
        ...
        if (fid->Vnode == 1 && fid->Unique == 1)
            ...
            AddILE(*inclist, name, fid->Vnode, fid->Unique,
                   ov->vptr->disk.vparent, ov->vptr->disk.uparent,
                   ov->vptr->disk.type);
        }
    }
}

static int CheckResolveRenameSemantics(rsle *r, Volume *volptr, ViceFid *dFid,
                                       dlist *vlist, vle **srcv, vle **tgtv,
                                       vle **srcdv, vle **tgtdv, olist *AllLogs,
                                       dlist *inclist, dlist *newinclist,
                                       int *blocks)
{
    ...
    *srcv = *tgtv = *srcdv = *tgtdv = 0;
    ...
    /* check that both src and target directories exist */
    {
        ...
        if (!FID_EQ(&OldDid, &NewDid)) {
            ...
            if (!npv || !npv->vptr) {
                LogMsg(0, SrvDebugLevel, stdout,
                       "ChkResRenSem: New Dir(%x.%x.%x) doesnt exist",
    ...
    // source object checks
    {
        ...
        if (SrcObjExists) {
            FormFid(spFid, V_id(volptr), sv->vptr->disk.vparent,
                    sv->vptr->disk.uparent);
            ...
            if (!SrcParentPtrOK) {
                ...
                LogMsg(
                    ...
                    spFid.Vnode, spFid.Unique, SrcFid.Vnode, SrcFid.Unique,
                    OldDid.Vnode, OldDid.Unique);
                AddToIncList(newinclist, vlist, volptr, &spFid, 0);
            }
        }

        if (!SrcNameExists || !SrcNameFidBindingOK || !SrcObjExists ||
            !SrcParentPtrOK) {
            LogMsg(0, SrvDebugLevel, stdout,
    ...
    //target object checks
    {
        if (!r->u.mv.tvnode && !r->u.mv.tunique) {
            ...
            if (tv && tv->vptr && !tv->vptr->delete_me)
                TgtObjExists = TRUE;
            if (TgtObjExists) {
                FormFid(tpFid, V_id(volptr), tv->vptr->disk.vparent,
                        tv->vptr->disk.uparent);
                TgtParentPtrOK = FID_EQ(&tpFid, &NewDid);
                if (!TgtParentPtrOK) {
                    LogMsg(0, SrvDebugLevel, stdout,
                           "ChkResRenSem: Parent(%x.%x) on tgtvnode(%x.%x)",
                           tpFid.Vnode, tpFid.Unique, TgtFid.Vnode,
                           TgtFid.Unique);
                    LogMsg(0, SrvDebugLevel, stdout,
                           "    doesn't match Target Dir %x.%x", NewDid.Vnode,
                           NewDid.Unique);
                    AddToIncList(newinclist, vlist, volptr, &tpFid, 0);
                }
            }
            if (!TgtNameExists || !TgtNameFidBindingOK || !TgtObjExists ||
                !TgtParentPtrOK) {
                LogMsg(0, SrvDebugLevel, stdout,
                       "ChkResRenSem: Tgt NE %d FBindOK %d OE %d PPOK %d",
                       TgtNameExists, TgtNameFidBindingOK, TgtObjExists,
                       TgtParentPtrOK);
                LogMsg(
                    0, SrvDebugLevel, stdout,
                    "ChkResRenSem: Marking both Old(%x.%x) and New dirs(%x.%x) inc",
            ...
            //check for remove update conflicts on target
            {
                if (!ISDIR(TgtFid)) {
                    ...
                    if (rup.rcode) {
                        AddILE(*newinclist, r->name2, TgtFid.Vnode,
                               TgtFid.Unique, NewDid.Vnode, NewDid.Unique,
                               vDirectory);
                        errorCode = EINCONS;
                        goto Exit;
                    }
    ...
    if ((errorCode = CheckRenameSemantics(
             NULL, &opv->vptr, &npv->vptr, &sv->vptr, r->name1,
             tv ? &tv->vptr : 0, r->name2, &volptr, 0, NULL, NULL, NULL, NULL,
                             ...
                             VolumeId VSGVolnum, olist *AllLogs, int *blocks)
{
    ...
    if (DH_IsEmpty(tdh)) {
        LogMsg(0, SrvDebugLevel, stdout,
               "CleanRenameTarget: Target %x.%x is already empty", tFid.Vnode,
               tFid.Unique);
        VN_PutDirHandle(tv->vptr);
        return (0);
    }
    TreeRmBlk pkdparm;
    pkdparm.init(0, VSGVolnum, volptr, 0, &r->storeid, vlist, 1, AllLogs,
                 r->index, blocks);
