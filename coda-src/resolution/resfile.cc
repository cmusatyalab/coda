...
long FileResolve(res_mgrpent *mgrp, ViceFid *Fid, ViceVersionVector **VV)
{
    ...
    /* fetch dominant file */
    {
	   ...
	   implement the uni-connection groups  --- PUNEET */
        errorcode =
            Res_FetchFile(mgrp->rrcc.handles[dix], Fid, 0, &Status, &sid);
        if (errorcode) {
                  ...
                  ResStatus *Status, SE_Descriptor *BD)
{
    ...
    if (vptr->disk.node.inodeNumber)
        ...
        tmpinode = icreate(V_device(volptr), V_id(volptr), vptr->vnodeNumber,
                           vptr->disk.uniquifier, 0);
    ...
    if ((errorcode = RPC2_InitSideEffect(RPCid, &sid)) <= RPC2_ELIMIT) {
        SLog(0, "RS_FetchFile: InitSideEffect failed %d", errorcode);
        errorcode = EINVAL;
        goto FreeLocks;
    }
    ...
    if ((errorcode = RPC2_CheckSideEffect(RPCid, &sid, SE_AWAITLOCALSTATUS)) <=
        ...
        if (errorcode == RPC2_SEFAIL1)
            errorcode = EIO;
        SLog(0, "RS_FetchFile: CheckSideEffect failed %d", errorcode);
        goto FreeLocks;
    }
    /* set OUT parameters */
...
FreeLocks:
    ...
    if (vptr) {
        if (tmpinode) {
            ...
            CODA_ASSERT(
                !(idec(V_device(volptr), tmpinode, V_parentId(volptr))));
        }
        Error filecode = 0;
                  ...
                  SE_Descriptor *BD)
{
    ...
    if (!XlateVid(&Fid->Volume)) {
        SLog(0, "RS_ForceFile: Couldnt Xlate VSG %x", Fid->Volume);
        return (EINVAL);
    }

    /* get object */
    if ((errorcode =
             GetFsObj(Fid, &volptr, &vptr, WRITE_LOCK, NO_LOCK, 0, 0, 0))) {
    ...
    if (res != VV_EQ) {
        ...
        newinode = icreate(V_device(volptr), V_id(volptr), vptr->vnodeNumber,
                           vptr->disk.uniquifier, vptr->disk.dataVersion + 1);
        CODA_ASSERT(newinode > 0);

        /* adjust the disk block count by the difference in the files */
        if ((errorcode = AdjustDiskUsage(
                 volptr, (nBlocks(Length) - nBlocks(vptr->disk.length))))) {
        ...
        /* fetch the file to be forced */
        {
            ...
            if ((errorcode = RPC2_InitSideEffect(RPCid, &sid)) <= RPC2_ELIMIT) {
                ChangeDiskUsage(volptr,
                                (nBlocks(vptr->disk.length) - nBlocks(Length)));
            ...
            if ((errorcode = RPC2_CheckSideEffect(
                ...
                ChangeDiskUsage(volptr,
                                (nBlocks(vptr->disk.length) - nBlocks(Length)));
                SLog(0, "RS_ForceFile: Error %d in CheckSideEffect", errorcode);
                goto FreeLocks;
            }
