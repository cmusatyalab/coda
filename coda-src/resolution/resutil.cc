...
long RS_NewConnection(RPC2_Handle RPCid, RPC2_Integer set, RPC2_Integer sl,
...
long RS_MarkInc(RPC2_Handle RPCid, ViceFid *Fid)
{
    ...
    errorcode =
        GetFsObj((ViceFid *)Fid, &volptr, &vptr, WRITE_LOCK, NO_LOCK, 0, 0, 0);
    if (errorcode)
        goto FreeLocks;
...
long CheckRetCodes(RPC2_Integer *rc, unsigned long *rh, unsigned long *hosts)
{
    ...
    for (int i = 0; i < VSG_MEMBERS; i++) {
        ...
        if (rh[i] && rc[i] && rc[i] != ETIMEDOUT) {
            ...
            SLog(
                0,
                "CheckRetCodes - an accessible server returned an error (server %s error %d)",
                inet_ntoa(addr), rc[i]);
        }
    }
    return (error);
              ...
              long punique, long type)
{
    ...
    if (dl.IsMember(il)) {
        ...
        SLog(39, "%s (%x.%x) child of (%x.%x),  type = %d", name, vnode, unique,
             pvnode, punique, type);
        dl.insert(il);
    }
    return (il);
...
void BSToDlist(RPC2_BoundedBS *BS, dlist *dl)
{
    ...
    while (beginc < endc) {
        ...
        long pvnode, punique;
        long type;
        ParseIncBSEntry(&beginc, &name, &vnode, &unique, &pvnode, &punique,
                        &type);
...
void ParseIncBSEntry(char **c, char **name, long *vn, long *unique, long *pvn,
                     long *punique, long *type)
{
    long *l = (long *)(*c);
    *c += sizeof(long);
                     ...
                     ViceFid *pFid, long type)
{
    ...
    SLog(39, "Entry: %s (%x.%x) child of %x.%x type %d", name, Fid->Vnode,
         Fid->Unique, pFid->Vnode, pFid->Unique, type);
...
int CompareIlinkEntry(ilink *i, ilink *j)
{
    ...
    if (i->vnode > j->vnode)
        return (1);
    if (i->unique < j->unique)
        return (-1);
    if (i->unique > j->unique)
        return (1);
    if (i->pvnode < j->pvnode)
        return (-1);
    if (i->pvnode > j->pvnode)
        return (1);
    if (i->punique < j->punique)
        return (-1);
    if (i->punique > j->punique)
        return (1);
