...
static char *rcsid = "$Header: /afs/cs.cmu.edu/project/coda-braam/src/coda-4.0.1/RCSLINK/./coda-src/rpc2/sl.c,v 1.1 1996/11/22 19:07:41 braam Exp $";
#endif /*_BLURB_*/
...
/*
	...
	Autonomous LWP that loops forever.
	Effectively the "bottom half" of the RPC code.
	Driven by timeouts and packet arrivals on the request socket.
	Communicates with "top half" routines via SL entries.
	
	ALL changes to NextSeqNumber field of connections take place entirely
    	within rpc2_SocketListener(). Increments (+2) are performed by
	rpc2_IncrementSeqNumber(), which may be called in the MultiRPC case by
	RPC2_MultiRPC() (if SocketListener return code is WAITING) or by 
	rpc2_MSendPacketsReliably() on client-initiated timeout.
...
PRIVATE RPC2_PacketBuffer *PullPacket(), *ShrinkPacket();
PRIVATE struct CEntry *MakeConn(), *FindOrNak();
PRIVATE struct SL_Entry *FindRecipient();
PRIVATE bool PoisonPacket(), BogusSl(), MorePackets(), PacketCame();
PRIVATE void
...
rpc2_SocketListener()
    {
    LWP_DispatchProcess();  /* just once, at RPC2_Init time, to be nice */
...
void rpc2_ProcessPackets()
    ...
    /* We are guaranteed that there is a packet in the socket buffer at this point */
    pb = PullPacket();
    if (pb == NULL) return;
    if (PoisonPacket(pb))  return;
    ...
    if (ntohl(pb->Header.RemoteHandle) == NULL)
    ...
    /* stamp the packet and update the host entry if there is one */
    if (ce && ce->HostInfo) 
	FT_GetTimeOfDay(&ce->HostInfo->LastWord, (struct timezone *)0);

    /* See if smaller packet will do */
    pb = ShrinkPacket(pb);

    if (pb->Header.Lamport  > rpc2_LamportClock)
	rpc2_LamportClock = pb->Header.Lamport + 1;	/* maintain causality */
...
void rpc2_ExpireEvents()
	...
	if (sl->Type == REPLY) FreeHeld(sl);
	else LWP_NoYieldSignal((char *)sl);
	}
    }


PRIVATE bool MorePackets()
    {
    struct timeval tv;
    ...
    if (select(8*sizeof(long), &rmask, &wmask, &emask, &tv) > 0) return(TRUE);
    else return(FALSE);
    }


PRIVATE bool PacketCame()
    ...
    emask = rmask = (1 << rpc2_RequestSocket);
    wmask = 0;
    if (IOMGR_Select(8*sizeof(long), &rmask, &wmask, &emask, tvp) == 1) return(TRUE);
    else return(FALSE);
    }

PRIVATE RPC2_PacketBuffer *PullPacket()
    {
    RPC2_PacketBuffer *pb;

    RPC2_AllocBuffer(RPC2_MAXPACKETSIZE-sizeof(RPC2_PacketBuffer), &pb);
    if (rpc2_RecvPacket(rpc2_RequestSocket, pb, &rpc2_ThisHost, &rpc2_ThisPortal) < 0)
	{
...
PRIVATE void Tell(pb, ce)
    register RPC2_PacketBuffer *pb;
    register struct CEntry *ce;
    {
#ifdef RPC2DEBUG
    if (RPC2_DebugLevel < 10) return;

    printf("Received packet....\n");
    rpc2_PrintPacketHeader(pb, stdout);

    if (ce == NULL) return;
    printf("Connection 0x%lx state is ...\n",  ce->UniqueCID);
    rpc2_PrintCEntry(ce, stdout);	    
#endif RPC2DEBUG
    }


PRIVATE void HandleSLPacket(pb)
    RPC2_PacketBuffer *pb;
    {
    register struct CEntry *ce;
    ...
    if (ce == NULL) {BOGUS(pb, "HandleSLPacket: ce == NULL\n");return;}
	...
	case RPC2_NAKED:
		...
		{HandleNak(pb, ce);}
	    else {BOGUS(pb, "HandleSLPacket: state != AWAIT\n");}
	    break;
...
PRIVATE struct CEntry *FindOrNak(pb)
    RPC2_PacketBuffer *pb;
    {
    register struct CEntry *ce;
...
PRIVATE void DecodePacket(pb, ce)
    RPC2_PacketBuffer *pb;
    register struct CEntry *ce;
    {
    switch ((int) pb->Header.Opcode)
...
PRIVATE RPC2_PacketBuffer *ShrinkPacket(pb)
    RPC2_PacketBuffer *pb;
    {
    RPC2_PacketBuffer *pb2;
...
PRIVATE void SendBusy(ce, doEncrypt)
    register struct CEntry *ce;
...
PRIVATE void HandleCurrentReply(pb, ce)
    RPC2_PacketBuffer *pb;
    register struct CEntry *ce;
    {
    register struct SL_Entry *sl;
...
PRIVATE void HandleNewRequest(pb, ce)
    RPC2_PacketBuffer *pb;
    register struct CEntry *ce;
    {
    register struct SL_Entry *sl;
...
PRIVATE struct SL_Entry *FindRecipient(pb)
    register RPC2_PacketBuffer *pb;
    {
    register long i;
    register struct SL_Entry *sl;
	...
	else sl = sl->NextEntry;
	}
    return(NULL);
    }

PRIVATE void HandleCurrentRequest(pbX, ceA)
    RPC2_PacketBuffer *pbX;
    register struct CEntry *ceA;
    {
...
PRIVATE void HandleRetriedBind(pb, ce)
    RPC2_PacketBuffer *pb;
    register struct CEntry *ce;    
    {
    ...
    */
	...
	rpc2_XmitPacket(rpc2_RequestSocket, ce->HeldPacket, &ce->PeerHost,
	           &ce->PeerPortal);
	RPC2_FreeBuffer(&pb);
	return;
	}
...
PRIVATE void HandleInit2(pb, ce)
    RPC2_PacketBuffer *pb;
    register struct CEntry *ce;
    {
    register struct SL_Entry *sl;
    ...
    sl = ce->MySl;
    sl->Packet = pb;
    if (ce->SecurityLevel == RPC2_OPENKIMONO)  	SetState(ce, C_THINK);
    else SetState(ce, C_AWAITINIT4);
    rpc2_DeactivateSle(sl, ARRIVED);
    LWP_NoYieldSignal((char *)sl);
    }

PRIVATE void HandleInit4(pb, ce)
    RPC2_PacketBuffer *pb;
    register struct CEntry *ce;
    {
    register struct SL_Entry *sl;
...
PRIVATE void HandleInit3(pb, ce)
    RPC2_PacketBuffer *pb;
    register struct CEntry *ce;
    {
    register struct SL_Entry *sl;
	...
	else say(0, RPC2_DebugLevel, ("Bogus Init3\n"));
...
PRIVATE void SendNak(remoteHandle, whichHost, whichPortal)
    register RPC2_Handle remoteHandle;
...
PRIVATE struct CEntry *MakeConn(pb)
    struct RPC2_PacketBuffer *pb;
    /* Make a new server connection for incoming Init1 packet pb. Returns pointer
	to the connection made.
	FakeBody fields are left in network order for RP2Gen code to deal with properly.
	See comment in RPC2_GetRequest() about this.  All other fields of Init1Body
	are converted to host order.
	Returns NULL without allocating connection if packet turns out to be bogus in 
	some way */
    {
    ...
    if ( (pb->Header.BodyLength < sizeof(struct Init1Body) - sizeof(ib1->Text))
	...
	say(0, RPC2_DebugLevel, ("Ignoring INIT1 packet with invalid Tag fields\n"));
	return(NULL);
	}
    ...
    if (RPC2_DebugLevel > 9)
	{
	printf("New Connection 0x%lx......\n",  ce);
	rpc2_PrintCEntry(ce, stdout);(void) fflush(stdout);
	}
#endif RPC2DEBUG

    rpc2_NoteBinding(&rpc2_ThisHost, &rpc2_ThisPortal, pb->Header.Uniquefier, ce->UniqueCID);
    return(ce);
    }

void rpc2_IncrementSeqNumber(ce)
    struct CEntry *ce;
    {
    ce->NextSeqNumber += 2;
    }
...
PRIVATE void HandleOldRequest(pb, ce)
    RPC2_PacketBuffer *pb;
    register struct CEntry *ce;
    {
