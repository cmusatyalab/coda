...
# verify
# 
print "Dumping done.\n\nNow verifying the tape:\n";
system("mt -f $tape rewind");
$partno = 1;
foreach $part ( @partitions ) { 
    print "\n\n---------> Partition $part:\n";
    print     "---------> command: restore -s $partno -f $tape -t /\n";
    $rc = 0xffff & system("restore -s $partno -f $tape -t /");
    if ( $rc ) { 
...
sub VerifyTape {
    ...
    open( CFD , "|restore -x -f $tape TAPELABEL");
    print CFD "1\nn\n";
    close CFD;
    printf "Done.\n";
    if ( -f "/tmp/TAPELABEL" ) {
