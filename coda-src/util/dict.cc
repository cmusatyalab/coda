...
static char *rcsid = "$Header: /coda/coda.cs.cmu.edu/project/coda/cvs/coda/coda-src/util/dict.cc,v 4.3 1998/01/10 18:38:29 braam Exp $";
#endif /*_BLURB_*/
...
void assocrefs::Attach(assoc *Assoc, int ix) {
	...
	if (assocs == 0) {
	    ...
	    bzero((const void *)assocs, NewMax * sizeof(assoc *));
	}
	else {
	    assocs = (assoc **)realloc(assocs, NewMax * sizeof(assoc *));
	    bzero((const void *)assocs + max - ActualGrowSize, ActualGrowSize * sizeof(assoc *));
	}
