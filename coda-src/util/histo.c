...
int InitHisto(struct hgram *hg, double lolimit, double hilimit, int bucketcount,
              enum htype ht)
       ...
       number -- number of newvals to be entered */
{
    ...
    for (i = 0; i < hg->maxb; i++) { /* I know I could use binary search here */
        if (newval >= hg->buckets[i].hival)
            continue;
...
/* dFreedom:  no of degrees of freedom; == (NoOfSamples - 1) */
{
    ...
    static double lowCI[20] = { -1.0,  3.078, 1.886, 1.638, 1.533, 1.476, 1.440,
                                1.415, 1.397, 1.383, 1.372, 1.363, 1.356, 1.35,
                                1.345, 1.341, 1.337, 1.333, 1.33,  1.328 };
...
int PrintHisto(FILE *outfile, struct hgram *hg)
{
    ...
    if (hg->count < 2)
        ...
        fprintf(outfile, "Count = %d, Sum = %g, Sum2 = %g\n", hg->count,
                hg->sum, hg->sum2);
        fflush(outfile);
        temp = ((double)hg->count * hg->sum2 - hg->sum * hg->sum) /
               ((double)hg->count * (double)(hg->count - 1));
        CODA_ASSERT(temp >= 0);
        stddev = sqrt(temp);
    ...
    if (fprintf(
            outfile,
            "Good samples:     count = %d    mean = %g   stddev = %g   90%%CI = %g\n\n",
            hg->count, mean, stddev, c90) == -1)
        return -1;
    if (fprintf(outfile,
                "Bad samples:      underflow = %d    overflow = %d\n\n",
                hg->uflow.count, hg->oflow.count) == -1)
        return -1;

    if (hg->uflow.count)
        if (fprintf(outfile,
                    "        %d samples in the range -INFINITY to %g\n",
                    hg->uflow.count, hg->uflow.hival) == -1)
            return -1;

    for (i = 0; i < hg->maxb; i++) {
        if (hg->buckets[i].count)
            if (fprintf(outfile, "        %d samples in the range %g to %g\n",
                        hg->buckets[i].count, hg->buckets[i].loval,
                        hg->buckets[i].hival) == -1)
                return -1;
    }

    if (hg->oflow.count)
        if (fprintf(outfile, "        %d samples in the range %g to INFINITY\n",
                    hg->oflow.count, hg->oflow.loval) == -1)
            return -1;
    return 0;
}
...

{
    ...
    for (i = 0; i < hg->maxb; i++) {
        ...
        if (hg->buckets[i].count > maxcount)
            maxcount = hg->buckets[i].count;
    }
    ...
    for (i = 0; i < hg->maxb; i++) {
        if (fprintf(outfile, "%g %g\n", hg->buckets[i].loval,
                    (hg->buckets[i].count * 100.0) / (totalcount * 1.0)) == -1)
            return -1;
    }
    if (fprintf(outfile, "\n") == -1)
        return -1;
    ...
    if (fprintf(outfile, "Y Maximum %g\n",
                ceil((maxcount * 100.0) / (totalcount * 1.0))) == -1)
        return -1;
