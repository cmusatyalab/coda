...
static char *rcsid = "$Header: /afs/cs/project/coda-src/cvs/coda/coda-src/util/Attic/timing_paths.c,v 4.2 1998/08/05 23:50:12 braam dead $";
...
static init_new_pth(thistie, pth_ptr, curr_add)
    struct tie *thistie;
    struct pth *pth_ptr;
    int curr_add;
...
static compare_pth( thistie,pinfo, curr_add)
    struct tie *thistie;
    struct pths_info *pinfo;
    int curr_add;
...
static update_old_pth( pth_ptr, curr_add)
    struct pth *pth_ptr;
    int curr_add;
...
static create_new_pth(thistie, pinfo, curr_add)
    struct tie *thistie;
    struct pths_info *pinfo;    
    int curr_add;
    {
...
static calc_pth_lngth(thistie, curr_add)
    struct tie *thistie;
    int curr_add;
...
static entry_cmp ( thistie, entry1, entry2, entry_count)
...
static gen_col( thistie, col, prb_num, pth_ptr)
    struct tie *thistie;
    struct pth *pth_ptr;
    int prb_num, *col;
...
static call_histo( col_val,num_elm, fp)
    int *col_val;
    FILE *fp;
...
static int min(ar_ptr, num_elements)
    int *ar_ptr;
    int num_elements;
...
static int max(ar_ptr, num_elements)
    int *ar_ptr;
    int num_elements;
...
static evaltime( i, thistie,tptr)
    int i;
    struct tie *thistie;
    struct timeval *tptr;
...
static process_entries(thistie, tptr)
    struct tie *thistie;
    struct timeval *tptr;
