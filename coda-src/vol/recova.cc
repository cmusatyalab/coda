...
int NewVolHeader(struct VolumeHeader *header, Error *err)
{
    ...
    while (1) {
        for (i = 0; i < MAXVOLS; i++) {
            if (SRV_RVM(VolumeList[i]).header.stamp.magic !=
                VOLUMEHEADERMAGIC) {
                break;
            }
        }
    ...
    data.volumeInfo =
        (VolumeDiskData *)rvmlib_rec_malloc(sizeof(VolumeDiskData));
    ...
    data.smallVnodeLists =
        (rec_smolist *)rvmlib_rec_malloc(sizeof(rec_smolist) * SMALLGROWSIZE);
    ...
    data.largeVnodeLists =
        (rec_smolist *)rvmlib_rec_malloc(sizeof(rec_smolist) * LARGEGROWSIZE);
...
int VolHeaderByIndex(int myind, struct VolumeHeader *header)
{
    ...
    if ((myind < 0) || (myind >= (int)maxid) || (myind >= MAXVOLS)) {
        VLog(
            1,
            "VolHeaderByIndex: bogus volume index %d - maxid %d (ok if volume was purged or deleted)",
            myind, maxid);
        return (-1);
    }
...
int DeleteVolume(Volume *vp)
{
    ...
    RVMLIB_MODIFY(SRV_RVM(VolumeList[myind]).data.volumeInfo->destroyMe,
                  DESTROY_ME);
    RVMLIB_MODIFY(SRV_RVM(VolumeList[myind]).data.volumeInfo->blessed, 0);
    ...
    if ((status = DeleteVnodes(myind, dev, vSmall)) != 0)
        return status;
    if ((status = DeleteVnodes(myind, dev, vLarge)) != 0)
        return status;
    if ((status = DeleteVolData(myind)) != 0)
        return status;
    if ((status = DeleteVolHeader(myind)) != 0)
        return status;
...
int DeleteRvmVolume(unsigned int myind, Device dev)
{
    ...
    VLog(9, "Entering DeleteRvmVolume for volume %x",
         SRV_RVM(VolumeList[myind].header.id));
...
static int DeleteVnodes(unsigned int myind, Device dev, VnodeClass vclass)
{
    ...
    if (vdata->volumeInfo == NULL)
        return -1; /* WRONG! no VolumeDiskData! */
    ...
    while (moreVnodes) {
        ...
        while (count < MaxVnodesPerTransaction) {
            ...
            if ((vdo->type != vNull) && (vdo->vnodeMagic != vcp->magic)) {
                VLog(0, "DeleteVnodes:VnodeMagic incorrect for vn %d", i);
                CODA_ASSERT(0);
            }
