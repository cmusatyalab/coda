...
static void CheckServers(int argc, char *argv[], int opslot)
{
    ...
    if (insrv)
        free(insrv); /* free insrv only if it was alloc before */
    downsrvarray = (struct in_addr *)piobuf;
    if (downsrvarray[0].s_addr == 0) {
        printf("All servers up\n");
        return;
    }
    ...
    for (i = 0; downsrvarray[i].s_addr != 0; i++) {
        ...
        if (hent)
            ...
            printf("  %s", inet_ntoa(downsrvarray[i]));
    }
    printf("\n");
}
...
static void ExamineClosure(int argc, char *argv[], int opslot)
{
    ...
    if (i < argc) {
        ...
        last      = argc - 1;
        filenames = argv;
    } else { /* No filenames specified on cmd line: look in /usr/coda/spool/<uid> */
        first = 0;
        last  = findclosures(&filenames) - 1;
        if (last < 0) {
            printf("No closures found\n");
            exit(-1);
        }
    }
   ...
   Returns 0 on success or non-fatal error, -1 on fatal error */
{
    ...
    if (!validateclosurespec(cloname, volname, volrootpath))
        return (0);
    ...
    case CLO_REPLAY:
        ...
        if (rc < 0) {
            fflush(stdout);
            printf("Couldn't replay closure\n");
            return (-1);
        }
        if (flags & CLO_REMOVE) {
            rc = unlink(cloname);
...
static void FlushObject(int argc, char *argv[], int opslot)
{
    ...
    for (i = 2; i < argc; i++) {
        if (argc > 3)
            printf("  %*s  ", w, argv[i]); /* echo input if more than one fid */
...
static void FlushASR(int argc, char *argv[], int opslot)
{
    ...
    for (i = 2; i < argc; i++) {
        if (argc > 3)
            printf("  %*s  ", w, argv[i]); /* echo input if >1 fid */
        /* Get its path */
        vio.in       = 0;
        vio.in_size  = 0;
        vio.out      = piobuf;
...
static int pioctl_GetFid(char *path, ViceFid *fid, char *realm,
                         ViceVersionVector *vv)
{
    ...
    if (vv)
        memcpy(vv, &out.vv, sizeof(ViceVersionVector));
...
static void GetPFid(int argc, char *argv[], int opslot)
{
    ...
    for (i = 2; i < argc; i++) {
        ...
        /* Validate next fid */
        char tmp;
        if (sscanf(argv[i], "%x.%x.%x@%c", &fid.Volume, &fid.Vnode, &fid.Unique,
                   &tmp) != 4) {
...
static void Help(int argc, char *argv[], int opslot)
{
    ...
    if (argc < 3) {
        ...
        for (i = 0; i < cmdcount; i++) {
            ...
            if (cmdarray[i].abbreviation)
                printf("  %3s  ", cmdarray[i].abbreviation);
            else
                printf("       ");
            printf("%s\n", cmdarray[i].helptxt);
        }
    ...
    if (helpop < 0)
        printf("Bogus opcode: type \"cfs help\" for list\n");
    else
 ...
 */
{
    ...
    if (a->pluscount > 0) {
        a->plusentries =
            (struct aclentry *)calloc(a->pluscount, sizeof(struct aclentry));
    ...
    if (a->minuscount > 0) {
        a->minusentries =
            (struct aclentry *)calloc(a->minuscount, sizeof(struct aclentry));
    ...
    for (i = 2; i < argc; i++) {
	   ...
	   cml_count, offlinemsg, motd, age, time, cml_bytes) */
        ...
        printf("  Status of volume %08x (%u) named \"%s\"\n", vs->Vid, vs->Vid,
               volname);
        if (*omsg)
            printf("  Offline message is \"%s\"\n", omsg);
        if (*motd)
            printf("  Message of the day is \"%s\"\n", motd);
        ...
        if (conn_state != Unreachable && local_only == 0) {
            ...
            if (vs->MaxQuota > 0)
                ...
                printf(" maximum quota is unlimited\n");
        ...
        if (cml_count)
            printf(
                "  There are %d CML entries pending for reintegration (%llu bytes)\n",
...
static void LookAside(int argc, char *argv[], int opslot)
{
    ...
    if (rc < 0) {
        PERROR("VIOC_LOOKASIDE");
        return;
    }

    if (piobuf[0])
        printf("%s", piobuf); /* result of lka command */
}
 ...
 */
{
    ...
    for (i = 2; i < argc; i++) {
        ...
        if (rc < 0 && errno == ENOENT) { /* It's a nonexistent name, alright! */
            rc = lstat(argv[i], &stbuf);
            if (stbuf.st_mode & 0xff00 &
                S_IFLNK) { /* It is a sym link; read it */
                ...
                if (piobuf[0] == '#')
                    printf(
                        "Dangling sym link; looks like a mount point for volume \"%s\"\n",
                        &piobuf[1]);
                else
                    printf("!! %s is not a volume mount point\n", argv[i]);
                continue;
            }
        }
...
static void RmMount(int argc, char *argv[], int opslot)
{
    ...
    for (i = 2; i < argc; i++) { /* Remove one mount point per iteration */

        if (argc > 3)
            printf("  %*s  ", w, argv[i]); /* echo input if more than one */
...
static void SetACL(int argc, char *argv[], int opslot)
{
    ...
    if (i < argc && (strcmp(argv[i], "-clear") == 0)) {
        clearflag = 1;
        i++;
    }
    ...
    if (clearflag && minusflag) {
        printf(
            "Sorry, I won't let you specify -clear and -negative together\n");
    ...
    while (i < argc) { /* Examine one pair per iteration */
        char *newid;
        int newrights, j;
        ...
        (*asize)++;
        if (*asize == 1)
            *aptr = (struct aclentry *)malloc(sizeof(struct aclentry));
        else
            (*aptr) = (struct aclentry *)realloc(
                *aptr, (*asize) * sizeof(struct aclentry));
    ...
    for (i = 0; i < a.pluscount; i++) {
        if (a.plusentries[i].rights != 0)
            sprintf(&piobuf[strlen(piobuf)], "%s %d\n", a.plusentries[i].id,
                    a.plusentries[i].rights);
    }

    for (i = 0; i < a.minuscount; i++) {
        if (a.minusentries[i].rights != 0)
            sprintf(&piobuf[strlen(piobuf)], "%s %d\n", a.minusentries[i].id,
                    a.minusentries[i].rights);
    }
...
static void SetVolume(int argc, char *argv[], int opslot)
{
    printf("Not supported by Coda yet\n");
}
...
static void WD(int argc, char *argv[], unsigned int age, unsigned int time)
{
    ...
    if (argc == 2) /* no more args -- do them all */
    {
        ...
        for (i = 2; i < argc; i++) {
            if (argc > 3)
                printf("  %*s\n", w,
                       argv[i]); /* echo input if more than one fid */
...
static void PreserveLocal(int argc, char *argv[], int opslot)
{
    ...
    if (rc) {
        PERROR("VIOC_REP_CMD(REP_CMD_PRESERVE)");
        exit(-1);
    }
}
...
static void ZoneLimit(int argc, char *argv[], int opslot)
{
  ...
  */
    ...
    if (rc < 0) {
        ...
        printf("Zone limits set:  red = %d  yellow = %d\n", rzlimit, yzlimit);
    }
}
       ...
       returns -1 if no such opcode */
{
    ...
    for (i = 0; i < cmdcount; i++) {
        if ((strcmp(s, cmdarray[i].opcode) == 0) ||
            (cmdarray[i].abbreviation &&
             strcmp(s, cmdarray[i].abbreviation) == 0))
            return (i);
    }
    return (-1);
}
