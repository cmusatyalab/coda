...
#define MAXNAMLEN CFS_MAXNAMLEN
#endif
...
do {                                                 \
printk("INSQUE: entered.\n"); \
printk("INSQUE: el at %d.\n", (int) &(el)); \
printk("INSQUE: head.back %d.\n", (int) (head).back); \
	(el).forw = ((head).back)->forw;             \
printk("INSQUE: el.forward assigned: %d.\n", (int) (el).forw); \
	(el).back = (head).back;                     \
printk("INSQUE: el.back assigned: %d.\n", (int)  (el).back); \
	((head).back)->forw = (struct queue *)&(el); \
printk("INSQUE: head.back.forw assigned: %d.\n", (int) ((head).back)->forw); \
	(head).back = (struct queue *)&(el);         \
printk("INSQUE: head.back assigned: %d.\n", (int) (head).back); \
} while (0)
#endif
...
/* communication wait/request/reply queues */
struct vcomm {
	u_long		vc_seq;
	SELPROC		vc_selproc;
 ...
 *
 * Explanation: the coda module in the kernel maintains an array of 
 * cfs_mntinfo's -- this is the table of coda mounted filesystems.
 * From a Linux vfs (maintained in an array of struct vfsmount)
 * we find the corresponding cfs_mntinfo by dereferencing 
 * a  coda_fs superblock in the vfsmount. This superblock points to the 
 * cfs_mount.
 *
 * Conversely, every element of the cfs_mount contains a ody_mnt, which
 * is a chain of pointers to vfsmounts -- for coda we could have used 
 * just a single vfsmount -- but Odyssey needs more. As remarked above, 
 * the ody_mount must be malloced.
 * 
 */
