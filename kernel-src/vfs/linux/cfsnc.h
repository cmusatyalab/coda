...
struct queue_elem { struct queue_elem *next, *prev; } ;

#define insque(elem, pred) do { __u32 flags; save_flags(flags); cli(); (elem)->next = (pred); (elem)->prev = (pred)->prev; (pred)->prev->next = (elem); (pred)->prev = (elem); restore_flags(flags);} while (0)
#define remque(elem) do {__u32 flags; save_flags(flags); cli(); (elem)->prev->next = (elem)->next; (elem)->next->prev = (elem)->prev; restore_flags(flags);} while(0)

#define CFSNC_HSHINS(elem, pred)	insque((struct queue_elem *)elem,(struct queue_elem *)pred)
#define CFSNC_HSHREM(elem)		remque((struct queue_elem *)elem)
#define CFSNC_HSHNUL(elem)		(elem)->hash_next = \
					(elem)->hash_prev = (elem)

#define CFSNC_LRUINS(elem, pred)	insque((struct queue_elem *)LRU_PART(elem), (struct queue_elem *)LRU_PART(pred))
#define CFSNC_LRUREM(elem)		remque((struct queue_elem *)LRU_PART(elem));
