...
static char *rcsid = "$Header: /afs/cs.cmu.edu/project/coda-braam/src/coda-4.0.1/RCSLINK/./lib-src/libcs/longarg.c,v 1.1 1996/11/22 19:19:40 braam Exp $";
#endif /*_BLURB_*/
...
long min,max,defalt;
{
	...
	if (*arg != '\0') {		/* if there was an arg */
		...
		if (*p) {
			...
			i = atol (arg);
			if (i<min || i>max) {
				fprintf (stderr,"%ld out of range.  ",i);
			}
		}
	}
