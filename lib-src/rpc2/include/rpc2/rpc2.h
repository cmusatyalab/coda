...
/*
The following constants are used to indicate the security-level of RPC
connections.
*/
...
Error:		Something went wrong, but the connection (if any)  is
                still usable.
...
/* Never used by RPC2 itself; used by rp2gen or higher levels to indicate bogus
 * data */
...
/*
Universal opcode values:  opcode values equal to or less than 0 are reserved.
Values greater than 0
are  usable by mutual agreement between clients and servers.
*/
#define RPC2_INIT1OPENKIMONO \
    -2 /* Begin a new connection with security level RPC2_OPENKIMONO */
#define RPC2_INIT1AUTHONLY \
    -3 /* Begin a new connection with security level RPC2_AUTHONLY */
#define RPC2_INIT1HEADERSONLY \
    -4 /* Begin a new connection with security level RPC2_HEADERSONLY */
#define RPC2_INIT1SECURE \
    -5 /* Begin a new connection with security level RPC2_SECURE */
...
/* size of the largest acceptable packet buffer in bytes
 * (includes prefix and header) */
...
{
    RPC2_HOSTBYNAME     = 39,
    RPC2_HOSTBYINETADDR = 17,
    RPC2_HOSTBYADDRINFO = 6,
    RPC2_DUMMYHOST      = 88888
} HostTag;

typedef enum
{
    RPC2_PORTBYINETNUMBER = 53,
    RPC2_PORTBYNAME       = 64,
    RPC2_DUMMYPORT        = 99999
} PortTag;

typedef enum
{
    RPC2_SUBSYSBYID   = 71,
    RPC2_SUBSYSBYNAME = 84,
    RPC2_DUMMYSUBSYS  = 44444
} SubsysTag;

typedef enum
{
    RPC2_MGRPBYINETADDR = 111,
    RPC2_MGRPBYADDRINFO = 121,
    RPC2_MGRPBYNAME     = 137,
    RPC2_DUMMYMGRP      = 77777
} MgrpTag;
...
A value of 0  turns  off the  output  altogether;  values  of  1, 10, and 100
...
A value of 1 turns on the printing, while 0 turns it off.  The default value for
this variable is 1.

RPC2_Trace controls the tracing of RPC calls, packet transmissions and packet
reception.  Set it to 1
for tracing.  Set to zero for stopping tracing.  The internal circular trace
buffer can be printed out
...
*/
typedef int32_t RPC2_Integer; /* 32-bit, 2's complement representation. On
                                 other machines, an explicit conversion may
                                 be needed. */
...
typedef struct {
    ...
    union {
        struct RPC2_addrinfo
            *AddrInfo; /* includes sockaddr, which includes port */
...
typedef struct /* data structure filled by RPC2_GetPeerInfo() call */
{
...
/*
The following data structure is the body of the packet synthesised by the
runtime system on a new connection, and returned as the result of an
RPC2_GetRequest().
*/
typedef struct {
...
typedef enum
{
    ...
    RPC2_BYTE_TAG           = 2,
    RPC2_STRING_TAG         = 3,
    RPC2_COUNTEDBS_TAG      = 4,
    RPC2_BOUNDEDBS_TAG      = 5,
    RPC2_BULKDESCRIPTOR_TAG = 6,
    RPC2_ENCRYPTIONKEY_TAG  = 7,
    RPC2_STRUCT_TAG         = 8,
    RPC2_ENUM_TAG           = 9,
    RPC2_DOUBLE_TAG         = 10
} TYPE_TAG;
...
typedef struct arg {
    ...
    void (*endlog)(long, RPC2_Integer, RPC2_Handle *,
                   RPC2_Integer *); /* used for stub logging */
} ARG;

/* Structure for passing multicast information */
typedef struct {
    RPC2_Handle Mgroup; /*  Multicast group handle*/
    RPC2_Integer ExpandHandle; /* flag indicating whether Mgroup handle should
                                  be expanded or not */
    RPC2_Integer StraySeen; /* on output, total number of stray responses */
    RPC2_Integer StrayLen; /* size of array StraySites[]; */
    RPC2_HostIdent *StraySites; /* on output, stray hosts that responded; some
                                   hosts are lost if StraySeen > StrayLen */
} RPC2_Multicast;
...
#ifndef NONDEBUG
#define RPC2_AllocBuffer(x, y) \
    (rpc2_AllocBuffer((long)(x), y, __FILE__, (long)__LINE__))
#else
#define RPC2_AllocBuffer(x, y) (rpc2_AllocBuffer((long)(x), y, 0, (long)0))
#endif /* NONDEBUG */
extern long rpc2_AllocBuffer(long MinBodySize, RPC2_PacketBuffer **BufferPtr,
                             const char *SrcFile, long SrcLine);
...
typedef long RPC2_UnpackMulti_func(int HowMany, RPC2_Handle ConnHandleList[],
                                   ARG_INFO *ArgInfo, RPC2_PacketBuffer *Reply,
...
extern long RPC2_MultiRPC(int HowMany, RPC2_Handle ConnHandleList[],
                          ...
                          RPC2_PacketBuffer *Request, SE_Descriptor SDescList[],
                          RPC2_UnpackMulti_func *, ARG_INFO *ArgInfo,
...
extern long RPC2_CheckSideEffect(RPC2_Handle ConnHandle, SE_Descriptor *SDesc,
                                 long Flags);
...
extern long
RPC2_CreateMgrp(RPC2_Handle *MgroupHandle, RPC2_McastIdent *MulticastHost,
...
extern long RPC2_RemoveFromMgrp(RPC2_Handle MgroupHandle,
                                RPC2_Handle ConnHandle);
...
extern long MRPC_MakeMulti(int ServerOp, ARG ArgTypes[], RPC2_Integer HowMany,
                           ...
                           RPC2_Multicast *MCast, RPC2_HandleResult_func *,
...
extern long RPC2_GetPeerLiveness(RPC2_Handle ConnHandle, struct timeval *Time,
                                 struct timeval *SETime);
extern long RPC2_GetNetInfo(RPC2_Handle ConnHandle, RPC2_NetLog *RPCLog,
                            RPC2_NetLog *SELog);
extern long RPC2_PutNetInfo(RPC2_Handle ConnHandle, RPC2_NetLog *RPCLog,
                            RPC2_NetLog *SELog);
...
struct SStats {
    unsigned long Total, /* PacketsSent */
...
struct RStats {
    unsigned long Total, /* PacketsRecvd */
