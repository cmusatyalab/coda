...
int main(int arg, char **argv)
{
    ...
    for (i = 'a'; i < 'z' + 1; i++) {
        j            = 6 * (i - 'a');
        ShortText[j] = ShortText[j + 1] = ShortText[j + 2] = ShortText[j + 3] =
            ShortText[j + 4] = ShortText[j + 5] = i;
    }
    LongText[0] = 0;
    for (i = 0; i < 10; i++)
    ...
    while (1) {
        ...
        case STOREFILE:
            ...
            *(Buff1->Body + 3 * sizeof(long)) = sed.Value.SmartFTPD.hashmark;
            ...
            tt = RPC2_MakeRPC(cid, Buff1, &sed, &Buff2, (struct timeval *)NULL,
                              (long)0);
            if (bwflag)
                fclose(BW_f);
#ifdef PROFILE
            ...
            if (tt == (long)SE_SUCCESS && sed.LocalStatus == SE_SUCCESS) {
                ...
                printf("RemoteStatus: %s\tLocalStatus: %s\n", SE_ErrorMsg(tt),
        ...
        if (Buff2 != NULL) {
            ...
            if (RPC2_DebugLevel > 1) {
                rpc2_htonp(
                    Buff2); /* rpc2_PrintPacketHeader assumes net order */
