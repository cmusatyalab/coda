...
static char *WhichMagic(int x)
{
    static char buf[20];
    switch (x) {
    case OBJ_PACKETBUFFER:
        return ("OBJ_PACKETBUFFER");
    case OBJ_CENTRY:
        return ("OBJ_CENTRY");
    case OBJ_SLENTRY:
        return ("OBJ_SLENTRY");
...
void rpc2_PrintFilter(RPC2_RequestFilter *fPtr, FILE *tFile)
{
    ...
    fprintf(tFile, "FromWhom = %s  OldOrNew = %s  ",
            fPtr->FromWhom == ANY ?
                "ANY" :
                (fPtr->FromWhom == ONECONN ?
                     "ONECONN" :
                     (fPtr->FromWhom == ONESUBSYS ? "ONESUBSYS" : "??????")),
            fPtr->OldOrNew == OLD ?
                "OLD" :
                (fPtr->OldOrNew == NEW ?
                     "NEW" :
                     (fPtr->OldOrNew == OLDORNEW ? "OLDORNEW" : "??????")));
    switch (fPtr->FromWhom) {
    case ONECONN:
        fprintf(tFile, "WhichConn = %#x", fPtr->ConnOrSubsys.WhichConn);
        break;
    case ONESUBSYS:
        fprintf(tFile, "SubsysId = %ld", fPtr->ConnOrSubsys.SubsysId);
        break;
    case ANY:
        break;
    }
...
void rpc2_PrintSLEntry(struct SL_Entry *slPtr, FILE *tFile)
{
    ...
    case REQ:
        fprintf(tFile, "\tElementType = REQ  Packet = %p  Filter==>  ",
                slPtr->data);
                        ...
                        RPC2_NetLogEntry *Log, FILE *tFile)
{
    ...
    fprintf(tFile, "\t%s Observation Log Entries = %d (%d kept)\n", what,
            NumEntries, RPC2_MAXLOGLENGTH);

    if (NumEntries < RPC2_MAXLOGLENGTH)
        head = 0;
    else
        head = NumEntries - RPC2_MAXLOGLENGTH;
    while (head < NumEntries) {
        ix = head & (RPC2_MAXLOGLENGTH - 1);
        switch (Log[ix].Tag) {
        case RPC2_MEASURED_NLE:
            fprintf(tFile,
                    "\t\tentry %d: %ld.%06ld, conn %#x, %d bytes, %d msec\n",
                    ix, Log[ix].TimeStamp.tv_sec, Log[ix].TimeStamp.tv_usec,
                    Log[ix].Value.Measured.Conn, Log[ix].Value.Measured.Bytes,
                    Log[ix].Value.Measured.ElapsedTime);
            break;
        case RPC2_STATIC_NLE:
            fprintf(tFile,
                    "\t\tentry %d: %ld.%06ld, static bandwidth %d bytes/sec\n",
                    ix, Log[ix].TimeStamp.tv_sec, Log[ix].TimeStamp.tv_usec,
...
void rpc2_PrintCEntry(struct CEntry *cPtr, FILE *tFile)
{
    ...
    if (TestRole(cPtr, CLIENT))
        ...
        case C_THINK:
            fprintf(tFile, "C_THINK");
            break;
        case C_AWAITREPLY:
            fprintf(tFile, "C_AWAITREPLY");
            break;
        case C_AWAITINIT2:
            fprintf(tFile, "C_AWAITINIT2");
            break;
        case C_AWAITINIT4:
            fprintf(tFile, "C_AWAITINIT4");
            break;
        case C_HARDERROR:
            fprintf(tFile, "C_HARDERROR");
            break;
        default:
            fprintf(tFile, "???????");
            break;
    ...
    if (TestRole(cPtr, SERVER))
        switch ((int)(cPtr->State & 0x0000ffff)) {
        case S_AWAITREQUEST:
            fprintf(tFile, "S_AWAITREQUEST");
            break;
        case S_PROCESS:
            fprintf(tFile, "S_PROCESS");
            break;
        case S_STARTBIND:
            fprintf(tFile, "S_STARTBIND");
            break;
        case S_FINISHBIND:
            fprintf(tFile, "S_FINISHBIND");
            break;
        case S_AWAITINIT3:
            fprintf(tFile, "S_AWAITINIT3");
            break;
        case S_REQINQUEUE:
            fprintf(tFile, "S_REQINQUEUE");
            break;
        case S_HARDERROR:
            fprintf(tFile, "S_HARDERROR");
            break;
        case S_INSE:
            fprintf(tFile, "S_INSE");
            break;
        case S_AWAITENABLE:
            fprintf(tFile, "S_AWAITENABLE");
            break;
        default:
            fprintf(tFile, "??????");
            break;
    ...
    fprintf(tFile, "\n\tSecurityLevel = %s",
            cPtr->SecurityLevel == RPC2_OPENKIMONO ?
                "RPC2_OPENKIMONO" :
                (cPtr->SecurityLevel == RPC2_AUTHONLY ?
                     "RPC2_AUTHONLY" :
                     (cPtr->SecurityLevel == RPC2_SECURE ?
                          "RPC2_SECURE" :
                          (cPtr->SecurityLevel == RPC2_HEADERSONLY ?
                               "RPC2_HEADERSONLY" :
                               "??????"))));
...
void rpc2_PrintMEntry(struct MEntry *mPtr, FILE *tFile)
{
    ...
    if (TestRole(mPtr, CLIENT)) {
        fprintf(tFile, "\n\tMaxlisteners = %ld  Listeners = %ld\n",
                mPtr->me_conns.me_client.mec_maxlisteners,
                mPtr->me_conns.me_client.mec_howmanylisteners);
    } else {
...
void rpc2_PrintPortIdent(RPC2_PortIdent *pPtr, FILE *tFile)
{
    ...
    case RPC2_PORTBYINETNUMBER:
        fprintf(tFile, "Port.InetPortNumber = %u",
                ntohs(pPtr->Value.InetPortNumber));
        break;
...
void rpc2_PrintSubsysIdent(RPC2_SubsysIdent *Subsys, FILE *tFile)
{
    ...
    case RPC2_SUBSYSBYNAME:
        say(-1, RPC2_DebugLevel,
            "Someone is still trying to use obsoleted RPC2_SUBSYSBYNAME\n");
        assert(0);
        break;
...
static char *CallName(int x)
{
    ...
    case EXPORT:
        return ("RPC2_Export");
    case DEEXPORT:
        return ("RPC2_DeExport");
    case ALLOCBUFFER:
        return ("RPC2_AllocBuffer");
    case FREEBUFFER:
        return ("RPC2_FreeBuffer");
    case SENDRESPONSE:
        return ("RPC2_SendResponse");
    case GETREQUEST:
        return ("RPC2_GetRequest");
    case MAKERPC:
        return ("RPC2_MakeRPC");
    case BIND:
        return ("RPC2_NewBinding");
    case INITSIDEEFFECT:
        return ("RPC2_InitSideEffect");
    case CHECKSIDEEFFECT:
        return ("RPC2_CheckSideEffect");
    case UNBIND:
        return ("RPC2_Unbind");
    case GETPRIVATEPOINTER:
        return ("RPC2_GetPrivatePointer");
    case SETPRIVATEPOINTER:
        return ("RPC2_SetPrivatePointer");
    case GETSEPOINTER:
        return ("RPC2_GetSEPointer");
    case SETSEPOINTER:
        return ("RPC2_SetSEPointer");
    case GETPEERINFO:
        return ("RPC2_GetPeerInfo");
    case SLNEWPACKET:
        return ("Packet Received");
    case SENDRELIABLY:
        return ("rpc2_SendReliably");
    case XMITPACKET:
        return ("rpc2_XmitPacket");
    case CLOCKTICK:
        return ("Clock Tick");
    case MULTIRPC:
        return ("RPC2_MultiRPC");
    case MSENDPACKETSRELIABLY:
        return ("rpc2_MSendPacketsReliably");
    case ADDTOMGRP:
        return ("RPC2_AddToMgrp");
    case CREATEMGRP:
        return ("RPC2_CreateMgrp");
    case REMOVEFROMMGRP:
        return ("rpc2_RemoveFromMgrp");
    case XLATEMCASTPACKET:
        return ("XlateMcastPacket");
    }
    return ("?????");
}
...
{
    long i;
    fprintf(outFile, "\nTrace Entry %ld:	<<<<<< %s: %s", whichIndex,
            whichTE->ActiveLWP, CallName(whichTE->CallCode));
    ...
    case EXPORT: {
        ...
        if (tea->Subsys.Tag == RPC2_SUBSYSBYID)
            fprintf(outFile,
                    "Subsys:	Tag = RPC2_SUBSYSBYID    SubsysId = %ld\n",
                    tea->Subsys.Value.SubsysId);
        else
            fprintf(outFile,
                    "Subsys:	Tag = RPC2_SUBSYSBYNAME  Name = \"%s\"\n",
    ...
    case GETREQUEST: {
        ...
        rpc2_PrintFilter(&tea->Filter, outFile);
        if (tea->IsNullBreathOfLife)
            fprintf(outFile, "BreathOfLife:  NULL\n");
        else
            fprintf(outFile, "BreathOfLife:	%ld.%ld\n",
    ...
    case MAKERPC: {
        ...
        if (tea->IsNullSDesc)
            ...
            fprintf(outFile, "\nSDesc: ");
            rpc2_PrintSEDesc(&tea->SDesc, outFile);
        }
        break; /* switch */
    }
    ...
    case BIND: {
        ...
        case RPC2_HOSTBYADDRINFO: {
            ...
            RPC2_formataddrinfo(tea->Host.Value.AddrInfo, addr,
                                RPC2_ADDRSTRLEN);
            fprintf(outFile,
                    "Host:     Tag = RPC2_HOSTBYINETADDR	AddrInfo = %s\n",
                    addr);
            break;
        }
        ...
        case RPC2_HOSTBYINETADDR:
            fprintf(outFile,
                    "Host:     Tag = RPC2_HOSTBYINETADDR	InetAddress = %s\n",
        ...
        case RPC2_PORTBYNAME:
            fprintf(outFile,
                    "Port:    Tag = RPC2_PORTBYNAME    Name = \"%s\"\n",
                    tea->Port.Value.Name);
            break;
        ...
        case RPC2_PORTBYINETNUMBER:
            fprintf(
                outFile,
                "Port:    Tag = RPC2_PORTBYINETNUMBER    InetNumber = \"%u\"\n",
        ...
        if (tea->IsNullClientIdent)
            fprintf(outFile, "ClientIdent:    NULL\n");
        else {
            long max;
            fprintf(outFile, "ClientIdent:    SeqLen = %u   SeqBody\"",
                    tea->ClientIdent.SeqLen);
            max = (tea->ClientIdent.SeqLen < sizeof(tea->ClientIdent_Value)) ?
                      tea->ClientIdent.SeqLen :
                      sizeof(tea->ClientIdent_Value);
            for (i = 0; i < max; i++)
                fprintf(outFile, "%c", (tea->ClientIdent_Value)[i]);
            if (max < tea->ClientIdent.SeqLen)
                fprintf(outFile, ".....");
            fprintf(outFile, "\"\n");
        }
        break; /* switch */
    ...
    case CHECKSIDEEFFECT: {
        ...
        if (tea->Flags & SE_AWAITLOCALSTATUS)
            fprintf(outFile, "SE_AWAITLOCALSTATUS  ");
        if (tea->Flags & SE_AWAITREMOTESTATUS)
            fprintf(outFile, "SE_AWAITREMOTESTATUS  ");
        fprintf(outFile, "}\n");
        break; /* switch */
    }
    ...
    case SENDRELIABLY: {
        ...
        if (tea->IsNullTimeout)
            fprintf(outFile, "TimeOut:    NULL\n");
        else
            fprintf(outFile, "TimeOut:	%ld.%ld\n", tea->Timeout.tv_sec,
                    tea->Timeout.tv_usec);
        break; /* switch */
    }
    ...
    case REMOVEFROMMGRP: {
        ...
        fprintf(outFile, "MEntry:      ");
        rpc2_PrintMEntry(&tea->me, outFile);
        fprintf(outFile, "        ");
        fprintf(outFile, "CEntry:      ");
        rpc2_PrintCEntry(&tea->ce, outFile);
        fprintf(outFile, "\n");
        break; /* switch */
    }
...
void rpc2_PrintSEDesc(SE_Descriptor *whichSDesc, FILE *whichFile)
{
    long i;
    if (whichFile == NULL)
        whichFile = rpc2_logfile; /* it's ok, call by value */
    for (i = 0; i < SE_DefCount; i++)
        if (SE_DefSpecs[i].SideEffectType == whichSDesc->Tag)
            break;
    if (i >= SE_DefCount)
        return; /* Bogus side effect */
