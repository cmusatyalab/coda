...
{
    HostHashTable =
        (struct HEntry **)calloc(HOSTHASHBUCKETS, sizeof(struct HEntry *));
                     ...
                     NetLogEntryType type)
{
    ...
    if (left > RPC2_MAXLOGLENGTH)
        left = RPC2_MAXLOGLENGTH;
    if (left > NumEntries)
        left = NumEntries; /* we have less */
                      ...
                      uint32_t bytes)
{
    ...
    if (current >= estimated)
        ...
        estimated -= (estimated - current) >> RPC2_BW_SHIFT;
    if (!estimated)
        estimated = 1;
    *BWlo = 1000000000 / estimated;
    ...
    if (current >= *BWhi)
        ...
        *BWhi -= (*BWhi - current) >> RPC2_BW_SHIFT;
}
                          ...
                          RPC2_Unsigned InBytes, RPC2_Unsigned OutBytes)
{
    ...
    if (RPC2_DebugLevel) {
        ...
        fprintf(rpc2_logfile, "uRTT: %s %u %u %u %u %u %u %u %u %u\n", addr,
                elapsed_us, OutBytes, InBytes, rto, rtt_lat, host->BWlo_out,
                host->BWhi_out, host->BWlo_in, host->BWhi_in);
    }
...
int RPC2_GetRTT(RPC2_Handle handle, unsigned long *RTT, unsigned long *RTTvar)
{
    ...
    if (RTT)
        *RTT = ce->HostInfo->RTT >> RPC2_RTT_SHIFT;
    if (RTTvar)
        *RTTvar = ce->HostInfo->RTTvar >> RPC2_RTTVAR_SHIFT;
