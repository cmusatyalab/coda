...
static int timeval_le(lua_State *L)
{
    ...
    l2c_totimeval(L, 1, &a);
    l2c_totimeval(L, 2, &b);
    lua_pushboolean(L, (a.tv_sec < b.tv_sec) || ((a.tv_sec == b.tv_sec) &&
                                                 (a.tv_usec <= b.tv_usec)));
    return 1;
}
...
static const struct luaL_Reg timeval_m[] = { { "__eq", timeval_eq },
                                             { "__le", timeval_le },
                                             { "__lt", timeval_lt },
                                             { "__add", timeval_add },
                                             { "__sub", timeval_sub },
...
{
    struct timeval tv;
    if (setup_function("rtt_update"))
        return;
    if (push_hosttable(he))
        return;
                          ...
                          uint32_t rx)
{
    ...
    if (setup_function("rtt_retryinterval"))
        return 0;
    if (push_hosttable(he))
        return 0;
...
{
    if (setup_function("rtt_getbandwidth"))
        return 0;
    if (push_hosttable(he))
        return 0;
    ...
    if (bw_tx)
        *bw_tx = (uint32_t)lua_tointeger(L, -2);
    if (bw_rx)
        *bw_rx = (uint32_t)lua_tointeger(L, -1);
    lua_pop(L, 2);
    return 1;
}
                   ...
                   struct timeval *tv)
{
    ...
    if (out)
        rc = setup_function("fail_delay_tx");
    else
        rc = setup_function("fail_delay_rx");
    if (rc)
        return 0;
