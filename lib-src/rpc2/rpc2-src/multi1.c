...
static void SetupPackets(HowMany, ConnHandleList, ceaddr, slarray, MCast,
	...
	if (thisconn && thisconn->SEProcs != NULL && thisconn->SEProcs->SE_MultiRPC1 != NULL)
	    {
...
*/
	...
	if (!MCast)
	    ...
	    rpc2_XmitPacket(rpc2_RequestSocket, PacketArray[thispacket],
			    ConnArray[thispacket]->HostInfo->Addr);
	    }
...
/*		    LWP_NoYieldSignal((char *)c_entry);*/
		...
		case TIMEOUT:
		    ...
		    rpc2_XmitPacket(rpc2_RequestSocket, PacketArray[thispacket], c_entry->HostInfo->Addr);
		    break;	/* switch */
