...
int aes_xcbc_mac_init(void **ctx, const uint8_t *key, size_t len)
{
    ...
    memset(tmp.u8, 0x01, sizeof(aes_block));
    aes_encrypt(&tmp, &tmp, &c);
    aes_encrypt_key(tmp.u8, sizeof(aes_block) * 8, &state->C1);

    memset(tmp.u8, 0x02, sizeof(aes_block));
    aes_encrypt(&tmp, &state->K2, &c);

    memset(tmp.u8, 0x03, sizeof(aes_block));
...
void aes_xcbc_mac_128(void *ctx, const uint8_t *buf, size_t len, aes_block *mac)
{
    ...
    aes_block tmp, *in = (aes_block *)buf;
