...
Modification by Jan Harkes <jaharkes@cs.cmu.edu>
   Define 'WORD' to use unsigned int for the 32-bit quantity.
...
{ /* pack bytes into a 32-bit Word */
    return ((WORD)b[3] << 24) | ((WORD)b[2] << 16) | ((WORD)b[1] << 8) |
           (WORD)b[0];
}
...
loop and hard-code in the values of fi[]  */
        ...
        for (m = j = 0; j < Nb;
             j++, m += 3) { /* deal with each 32-bit element of the State */
...
loop and hard-code in the values of ri[]  */
        ...
        for (m = j = 0; j < Nb;
             j++, m += 3) { /* This is the time-critical bit */
