...
extern device_t *rvm_errdev; /* last device reportiing error */
extern log_t *default_log; /* default log descriptor ptr */
extern rvm_length_t page_size; /* page length in bytes */
extern rvm_length_t page_mask; /* mask for rounding down to page size */
extern char *rvm_errmsg; /* internal error message ptr */

extern chk_vec_t *rvm_chk_vec; /* monitor range vector */
extern rvm_length_t rvm_chk_len; /* length of monitor range vector */
extern rvm_monitor_call_t *rvm_monitor; /* monitor call-back function ptr */
extern rvm_signal_call_t *rvm_chk_sigint; /* SIGINT handler call function ptr */
...
#define ALL_FORMATS                                                       \
    ((unsigned long)(char_sw | dec_sw | octal_sw | hex_sw | unsigned_sw | \
                     float_sw | double_sw | offset_sw | tv_sw))
                            ...
                            int *base_used /* base used for conversion [out] */)
{
    ...
    skip_white(ptr);
    if (*str == ',') { /* low order value follows */
        str++;
                      ...
                      long len /* maximum length */)
{
    ...
    while (!end_line(*cmd_cur) && (indx < len)) {
        ...
        else if (isoctal(*cmd_cur)) /* convert octal numerics */
        {
            (void)strcpy(ostr, "0   ");
            ostr[1] = *(cmd_cur++);
            if (isoctal(*cmd_cur)) {
...
static rvm_bool_t scan_num_format(void)
{
    ...
    DO_FOREVER
    {
        ...
        case 'L':
            num_format &= ~ALL_SIZES;
            num_format &= ~ALL_FORMATS;
            num_format |= (unsigned long)(long_sw | unsigned_sw | dec_sw);
            break;
        case 'o':
            num_format &= ~ALL_FORMATS;
                      ...
                      long count /* count to print */, rvm_bool_t all)
{
    ...
    case tv_sw:
        ...
        /* formats */
    case (unsigned long)(unsigned_sw | hex_sw):
    case hex_sw:
        putc('x', out_stream);
        break;
    case (unsigned long)(unsigned_sw | octal_sw):
    case octal_sw:
        putc('o', out_stream);
        break;
    case (unsigned long)(unsigned_sw | dec_sw):
    case unsigned_sw:
        putc('u', out_stream);
    case dec_sw:
        putc('d', out_stream);
...
static void pr_memory(FILE *out_stream, FILE *err_stream, char *addr,
                      rvm_length_t count, rvm_length_t *tot_size,
                      int *tot_width)
{
    ...
    case long_sw:
        ...
        break;
    case offset_sw:
        switch (num_format & (unsigned long)hex_sw & (unsigned long)octal_sw) {
        case octal_sw:
            base = 8;
            break;
        case hex_sw:
    ...
    while (count-- > 0) {
        switch (format) { /* determine format and print */
        case dec_sw:
            if (num_format & (unsigned long)unsigned_sw)
        ...
        case offset_sw:
            width = 0;
            prc_offset(out_stream, (rvm_offset_t *)&addr[i], base, &width);
            break;
        case char_sw:
            if (addr[i] == '\0')
                return;
        ...
        case tv_sw:
            width = 0;
            pr_timeval(out_stream, (struct timeval *)&addr[i], rvm_true,
                       &width);
            break;
        default:
            assert(rvm_false);
    ...
    char *limit_str /* msg to print if limit exceeded */)
{
    ...
    while (count > 0) {
        ...
        if (chk_sigint(out_stream))
            return rvm_true;
        if ((data_ptr = chk_buffer(offset, SECTOR_SIZE, err_stream)) == NULL)
            return rvm_false;
        ...
        while ((count != 0) &&
               (tot_width <=
                line_width)) { /* see if will exceed limits of valid data */
            if (RVM_OFFSET_GTR(RVM_ADD_LENGTH_TO_OFFSET(
                                   *offset, num_format_size(num_format)),
                               *limit_offset)) {
                if (!num_all)
            ...
            if ((num_format & (unsigned long)char_sw) && (*data_ptr == '\0')) {
                count = 0;
                break; /* done */
            }
            ...
            tot_size = 0;
            pr_memory(out_stream, err_stream, data_ptr, 1, &tot_size,
                      &tot_width);
...
static void get_redirect(FILE **out_stream, FILE **err_stream)
{
    ...
    *err_stream  = stderr;
    ...
    if (append)
        redir = fopen(dev_str, "a");
    else
        redir = fopen(dev_str, "w");
    if (redir != NULL) {
        *out_stream = redir; /* redirect std output */
        if (redir_err)
            *err_stream = redir; /* redirect std error */
    } else
...
static str_name_entry_t type_vec[MAX_TYPES] = /* type sizes vector */
    {
        ...
        { "LOG_DEV_STATUS_SIZE", (char *)LOG_DEV_STATUS_SIZE },
        { "log_dev_status_t", (char *)sizeof(log_dev_status_t) },
        { "log_seg_t", (char *)sizeof(log_seg_t) },
        { "LOG_SPECIAL_IOV_MAX", (char *)LOG_SPECIAL_IOV_MAX },
...
static rvm_bool_t do_sizeof(void)
{
    ...
    DO_FOREVER
    {
        ...
        /* print the size */
        switch ((long)type_vec[type_index].target) { /* interpret request */
        case ALL_KEY:
            for (type_index = 0; rvm_true; type_index++) {
                if (type_vec[type_index].str_name[0] == '\0')
                    break;
                ...
                default:
                    ...
                    if (strstr(type_vec[type_index + 1].str_name,
                        ...
                        type_vec[type_index + 1].str_name)
                        break;
                    /* print */
                    printf("  sizeof(%s) = %ld\n",
                           type_vec[type_index].str_name,
              ...
              FILE *err_stream /* error output stream */)
{
    ...
    if (RVM_OFFSET_GTR(end_offset, peekpoke.end_offset) ||
        ...
        read_length =
            RVM_OFFSET_TO_LENGTH(RVM_SUB_OFFSETS(end_offset, peekpoke.offset));
    ...
    peekpoke.ptr =
        RVM_OFFSET_TO_LENGTH(RVM_SUB_OFFSETS(*offset, peekpoke.offset));
    return rvm_true;
}
...
static rvm_bool_t do_peek(void)
{
    ...
    DO_FOREVER
    {
        ...
        if (!pr_data_range(out_stream, err_stream, 0, LINE_WIDTH, &offset,
                           &pr_offset, base, num_count, chk_peekpoke,
                           &peekpoke.dev.num_bytes, EOF_STR))
            goto err_exit;
    }
            ...
            rvm_offset_t *init_offset /* initial offset in file */)
{
    ...
    if (--num_count > 0) {
        ...
        repeat_len =
            RVM_OFFSET_TO_LENGTH(RVM_SUB_OFFSETS(*offset, *init_offset));
        if (repeat_len == 0)
            return rvm_true; /* nothing to do */
        copy_buf = malloc(repeat_len);
        if (copy_buf == NULL) {
            fprintf(stderr, "\n? Heap exhausted\n");
            return rvm_false;
        }
        if (!read_peekpoke(init_offset, repeat_len, rvm_true, stderr))
            goto err_exit;
        BCOPY(&peekpoke.buf[peekpoke.ptr], copy_buf, repeat_len);

        /* repeat the assignments */
        for (i = num_count; i > 0; i--) {
            if (RVM_OFFSET_GTR(RVM_ADD_LENGTH_TO_OFFSET(*offset, repeat_len),
                ...
                repeat_len = RVM_OFFSET_TO_LENGTH(
                    RVM_SUB_OFFSETS(peekpoke.dev.num_bytes, *offset));
                i = 0;
            }
            if (!read_peekpoke(offset, repeat_len, rvm_true, stderr))
                goto err_exit;
            BCOPY(copy_buf, &peekpoke.buf[peekpoke.ptr], repeat_len);
            *offset = RVM_ADD_LENGTH_TO_OFFSET(*offset, repeat_len);
        }
    }
    goto exit;
...
static rvm_bool_t do_poke(void)
{
    ...
    DO_FOREVER
    {
        ...
        if (!scan_poke_val(&temp, &scanned_str, &scanned_float, &dbl_float,
                           stderr))
            return rvm_false;
        ...
        if (scanned_str) {
            ...
            if (scanned_float)
                ...
                dbl_float = RVM_OFFSET_TO_LENGTH(temp);
                           ...
                           rec_hdr_t *rec_hdr /* generic record header */)
{
    switch (rec_hdr->struct_id) {
    case trans_hdr_id:
        fprintf(out_stream, "TRANSACTION");
        return;

    case log_seg_id:
        fprintf(out_stream, "SEGMENT DICTIONARY ENTRY");
        return;

    case log_wrap_id:
        fprintf(out_stream, "WRAP-AROUND MARKER");
        return;
                          ...
                          char *cur_msg /* currency message */)
{
    ...
    else if (!chk_hdr_currency(log, rec_hdr))
                          ...
                          rec_end_t *rec_end /* record end marker */)
{
    ...
    log_offset = RVM_ADD_LENGTH_TO_OFFSET(log_buf->offset, log_buf->ptr);
                        ...
                        nv_range_t *nv /* range header */)
{
    ...
    log_offset = RVM_ADD_LENGTH_TO_OFFSET(log_buf->offset, log_buf->ptr);
    fprintf(out_stream,
            "  Modification range:  %7.1lu   Log offset:", nv->range_num);
    prc_offset(out_stream, &log_offset, 10, NULL);
    putc('\n', out_stream);
    fprintf(out_stream, "    VM address:     %#10.1lx   Length:     %5.1lu\n",
            (long)nv->vmaddr, nv->length);
    fprintf(out_stream,
            "    Segment code:          %3.1lu   Offset:", nv->seg_code);
                            ...
                            trans_hdr_t *trans_hdr /* transaction header */)
{
    ...
    pr_bool(out_stream, ((trans_hdr->flags & FIRST_ENTRY_FLAG) != 0), NULL);
    fprintf(out_stream, "      Last record:        ");
    pr_bool(out_stream, ((trans_hdr->flags & LAST_ENTRY_FLAG) != 0), NULL);
    putc('\n', out_stream);

    fprintf(out_stream, "  Number ranges:      %8.1lu", trans_hdr->num_ranges);
                      ...
                      char *cur_msg /* currency message */)
{
    ...
    switch (rec_hdr->struct_id) {
    case trans_hdr_id:
        print_transaction(out_stream, err_stream, (trans_hdr_t *)rec_hdr);
        break;
    case log_seg_id:
    ...
    default:
        assert(rvm_false); /* log damage if we get here */
    }
...
static rec_hdr_t *set_log_position(void)
{
    ...
    cur_offset    = RVM_ADD_LENGTH_TO_OFFSET(log_buf->offset, log_buf->ptr);
...
static rvm_bool_t reset_buffer(rvm_bool_t new_direction)
{
    ...
    if (RVM_OFFSET_EQL_ZERO(cur_offset)) {
        fprintf(stdout, " -- no current record, use show head, tail, or");
...
static rvm_return_t load_sub_rec(long incr, rvm_bool_t direction)
{
    ...
    if ((tmp_ptr >= log_buf->length) ||
        (tmp_ptr < 0)) { /* no, must re-init buffer */
        if (tmp_ptr >= 0)
            offset = RVM_ADD_LENGTH_TO_OFFSET(log_buf->offset, tmp_ptr);
        else
            offset = RVM_SUB_LENGTH_FROM_OFFSET(log_buf->offset, -tmp_ptr);
    ...
    if (tmp_ptr > log_buf->length) {
        ...
        if (RVM_OFFSET_LSS(offset, log->status.log_start) ||
            RVM_OFFSET_GEQ(offset, log->dev.num_bytes))
            goto no_record; /* not in log */
        target = RVM_ADD_LENGTH_TO_OFFSET(log_buf->offset, log_buf->ptr);
        ...
        if (direction == REVERSE)
            log_buf->ptr =
                RVM_OFFSET_TO_LENGTH(RVM_SUB_OFFSETS(target, log_buf->offset));
    }
...
static rvm_bool_t load_rec_hdr(FILE *err_stream /* error output stream */)
{
    ...
    if (rec_end->rec_hdr.struct_id != log_wrap_id) {
        ...
        if (tmp_ptr < 0) {
            ...
            offset = RVM_SUB_LENGTH_FROM_OFFSET(log_buf->offset, -tmp_ptr);
            if ((retval = init_buffer(log, &offset, FORWARD, NO_SYNCH)) !=
                RVM_SUCCESS) {
                no_rec = rvm_true;
                                  ...
                                  FILE *err_stream /* error output stream */)
{
    ...
    DO_FOREVER
    {
        ...
        if (rec_end->rec_hdr.struct_id == rec_end_id) {
            offset =
                RVM_ADD_LENGTH_TO_OFFSET(offset, rec_end->rec_hdr.rec_length);
    ...
    if (rec_end->rec_hdr.struct_id != log_wrap_id)
        cur_offset =
            RVM_SUB_LENGTH_FROM_OFFSET(cur_offset, rec_end->rec_hdr.rec_length);
                               ...
                               struct timeval *ts /* timestamp if != NULL */)
{
    ...
    {
        if ((retval = init_buffer(log, &log->status.log_tail, REVERSE,
    ...
    while (((rec_num != 0) && (rec_num != cur_rec_num)) ||
        ...
        if (direction == FORWARD) {
            ...
            if (log_buf->ptr < 0) {
                ...
                offset =
                    RVM_SUB_LENGTH_FROM_OFFSET(log_buf->offset, log_buf->ptr);
                offset = RVM_ADD_LENGTH_TO_OFFSET(offset, sizeof(nv_range_t));
...
static rvm_return_t locate_prev_sub_rec(void)
{
    ...
    offset = RVM_ADD_LENGTH_TO_OFFSET(log_buf->offset, log_buf->ptr);
    if (RVM_OFFSET_EQL(offset,
                       log->status.log_start)) { /* yes, find wrap marker */
                              ...
                              key_id_t key /* NUM/NEXT/PREV */)
{
    ...
    if (num == 0)
        num = 1; /* always try to print 1 record */
    ...
    while (num > 0) {
        ...
        case NEXT_KEY: /* scan next record */
            if ((!RVM_OFFSET_EQL(cur_offset, status->log_head)) && no_rec)
        ...
        case PREV_KEY: /* scan previous record */
            if ((!RVM_OFFSET_EQL(cur_offset, status->log_tail)) && no_rec)
        ...
        default:
            assert(rvm_false); /* internal error */
        }
                           ...
                           key_id_t key /* ALL/REMAINING */)
{
    ...
    /* initialize buffer at head of log */
    if (key == ALL_KEY)
        if (!show_head(out_stream, err_stream))
            return rvm_false;
    if (log_buf->ptr == -1)
                         ...
                         FILE *err_stream /* error output stream */)
{
    ...
    DO_FOREVER
    {
        ...
        if ((retval = load_aux_buf(log, offset, length, &ptr, &len, NO_SYNCH,
                                   rvm_true)) != RVM_SUCCESS) {
            pr_rvm_error(err_stream, retval, "reading log");
            return NULL;
        }
             ...
             FILE *err_stream /* error output stream */)
{
    ...
    *err_sw    = rvm_false;
    ...
    if (!num_all) /* adjust offset for vmaddr */
        cmp_offset = RVM_ADD_LENGTH_TO_OFFSET(
            cmp_offset,
            (rvm_length_t)RVM_SUB_LENGTH_FROM_ADDR(vmaddr, nv->vmaddr));
    ...
    while (!scan_stop(*cmd_cur)) {
        ...
        if (scanned_str) /* string values */
        {
            len      = strlen(temp_str);
            scan_ptr = temp_str;
        } else if (scanned_float) /* float values */
        {
                            ...
                            FILE *err_stream /* error output stream */)
{
    ...
    if ((*cmd_cur == '=')) {
        ...
        mod_cur = cmd_cur;
        while (!scan_stop(*cmd_cur)) {
            if (!scan_poke_val(&temp, &err_sw, &err_sw, &dbl_float, stderr))
                return rvm_false;
        }
        if (mod_cur == cmd_cur) {
            fprintf(err_stream, "\n? No values specified\n");
    ...
    DO_FOREVER
    {
        ...
        putc('\n', out_stream);
        return pr_data_range(out_stream, err_stream, 2, LINE_WIDTH, &offset,
                             &temp, base, num_count, chk_aux_buf, &limit,
                             DATA_END_STR);
    next_sub_rec:;
    }
exit:
                                  ...
                                  FILE *err_stream /* error stream */)
{
    ...
    {
        size_temp = RVM_SUB_OFFSETS(log_dev->num_bytes, size_temp2);
...
static rvm_bool_t do_show(void)
{
    ...
    DO_FOREVER
    {
        ...
        case TAIL_KEY: /* print tail (last) record */
            if (!show_tail(out_stream, err_stream))
                goto err_exit;
            break;
        case MODS_KEY: /* print modifications of vm */
            if (!show_mods(out_stream, err_stream))
                goto err_exit;
            break;
        ...
        case UNAME_KEY: /* print rec selected by uname */
            ...
            break;
        case EARLY_KEY:
            if (!show_earliest(out_stream, err_stream))
                goto err_exit;
            break;
        case REMAINING_KEY:
               ...
               rvm_length_t *key_ptr /* index of key word [out] */)
{
    ...
    *key_ptr = scan_str_name(str_name_buf, set_key_vec, "key word");
    ...
    case HEAD_KEY:
        if (prev)
            *offset_ptr = &status->prev_log_head;
        else
            *offset_ptr = &status->log_head;
        return rvm_true;
    case LOG_START_KEY:
    ...
    case TAIL_KEY:
        if (prev)
            *offset_ptr = &status->prev_log_tail;
        else
            *offset_ptr = &status->log_tail;
        return rvm_true;
    default:
...
static rvm_bool_t do_set(void)
{
    ...
    if (isdigit(*cmd_cur) || (*cmd_cur == 'x') || (*cmd_cur == 'X'))
        ...
        if (!scan_set_field(&src, &len, &key_index))
            return rvm_false;
                                    ...
                                    char *prompt /* prompt string */)
{
    ...
    DO_FOREVER
    {
        ...
        DO_FOREVER
        {
            ...
            case 'K': /* size in k */
            {
                log_len = log_len * 1024;
                break;
            }
            case 'm':
            case 'M': /* size in megabytes */
            {
                log_len = log_len * 1048576;
                break;
            }
            case 's':
            case 'S': /* size in sectors */
            {
                log_len = log_len * SECTOR_SIZE;
                break;
            }
            case 'p':
            case 'P': /* size in pages */
            {
                log_len = log_len * page_size;
                break;
            }
            case 'b':
            case 'B':
            ...
            default: {
                ...
                read_prompt_line(
                    out_stream,
                    "Enter size code (b,k,m,s,p,<null>):", rvm_true);
                continue;
            }
            }
            break;
...
static rvm_bool_t do_init_log(void)
{
    ...
    log_len = LOG_DEV_STATUS_SIZE +
              CHOP_TO_SECTOR_SIZE(
                  get_file_length(stdout, "Enter length of log data area:"));

    /* build internal descriptors */
    if ((log = make_log(dev_str, &retval)) ==
        NULL) { /* can't make descriptor */
    ...
    while (RVM_OFFSET_LSS(offset, log_dev->num_bytes)) {
        wrt_len =
            RVM_OFFSET_TO_LENGTH(RVM_SUB_OFFSETS(log_dev->num_bytes, offset));
...
static rvm_bool_t do_find_earliest(void)
{
    ...
    if (!no_rec) {
        ...
        putchar('\n');
    } else if (!reset_buffer(cur_direction))
        return rvm_false;
               ...
               char *msg /* ptr to message test if not null */)
{
    ...
    if (vmaddr >= (rvm_length_t)rvm_chk_vec[i].vmaddr) {
        ...
        data_length =
            ((rvm_length_t)rvm_chk_vec[i].vmaddr + rvm_chk_vec[i].length) -
            vmaddr;
...
static rvm_bool_t do_monitor(void)
{
    ...
    DO_FOREVER
    {
        ...
        if (scan_stop(*cmd_cur)) {
            if (need_prompt) {
                if (!get_ans("  More ranges", rvm_false))
                    break;
        ...
        if ((rvm_chk_vec == NULL) || (rvm_chk_len == chk_alloc_len)) {
            chk_alloc_len += 5;
            rvm_chk_vec = (chk_vec_t *)REALLOC(
                rvm_chk_vec, chk_alloc_len * sizeof(chk_vec_t));
            if (rvm_chk_vec == NULL) {
                rvm_chk_len = 0;
        ...
        if (isdigit(*cmd_cur) || (*cmd_cur == 'x') || (*cmd_cur == 'X'))
            rvm_chk_vec[rvm_chk_len].vmaddr = (void *)str2ul(
                cmd_cur, &cmd_cur, &rvm_chk_vec[rvm_chk_len].radix);
        else {
...
static str_name_entry_t open_key_vec[MAX_OPEN_KEYS] = {
    { "file", (char *)FILE_KEY },     { "no_tail", (char *)NO_TAIL_KEY },
    { "tail", (char *)TAIL_KEY },     { "no_update", (char *)NO_UPDATE_KEY },
...
file_name:
    ...
    if ((retval = open_log(dev_str, &log, status_io, NULL)) != RVM_SUCCESS)
        ...
        case RVM_ELOG: {
            fprintf(stderr, "? Log status area damaged or uninitialized\n");
...
static rvm_bool_t do_copy_log(void)
{
    ...
    {
        read_prompt_line(
            stdout, "Enter name of destination file or device:", rvm_false);
        (void)scan_str(dev_str, MAXPATHLEN);
    ...
    printf("\n  Copying from %s to %s\n\n", default_log->dev.name, dev_str);

    /* set up destination status block */
    (void)BCOPY(status, &save_status, sizeof(log_status_t));
    if ((!log->dev.raw_io) &&
        (dest_dev.raw_io)) { /* convert from file to partition */
        dest_dev.num_bytes =
            RVM_SUB_LENGTH_FROM_OFFSET(log_dev->num_bytes, FILE_STATUS_OFFSET);
        dest_dev.num_bytes =
            RVM_ADD_LENGTH_TO_OFFSET(dest_dev.num_bytes, RAW_STATUS_OFFSET);
        status->log_start =
            RVM_MK_OFFSET(0, RAW_STATUS_OFFSET + LOG_DEV_STATUS_SIZE);
        status->log_head =
            RVM_SUB_LENGTH_FROM_OFFSET(status->log_head, FILE_STATUS_OFFSET);
        status->log_head =
            RVM_ADD_LENGTH_TO_OFFSET(status->log_head, RAW_STATUS_OFFSET);
        status->log_tail =
            RVM_SUB_LENGTH_FROM_OFFSET(status->log_tail, FILE_STATUS_OFFSET);
        status->log_tail =
            RVM_ADD_LENGTH_TO_OFFSET(status->log_tail, RAW_STATUS_OFFSET);
        if (!RVM_OFFSET_EQL_ZERO(status->prev_log_head)) {
            status->prev_log_head = RVM_SUB_LENGTH_FROM_OFFSET(
                status->prev_log_head, FILE_STATUS_OFFSET);
            status->prev_log_head = RVM_ADD_LENGTH_TO_OFFSET(
                status->prev_log_head, RAW_STATUS_OFFSET);
        }
        if (!RVM_OFFSET_EQL_ZERO(status->prev_log_tail)) {
            status->prev_log_tail = RVM_SUB_LENGTH_FROM_OFFSET(
                status->prev_log_tail, FILE_STATUS_OFFSET);
            status->prev_log_tail = RVM_ADD_LENGTH_TO_OFFSET(
                status->prev_log_tail, RAW_STATUS_OFFSET);
        }
    }
    /* convert from partition to file */
    else if ((log->dev.raw_io) && (!dest_dev.raw_io)) {
        dest_dev.num_bytes =
            RVM_SUB_LENGTH_FROM_OFFSET(log_dev->num_bytes, RAW_STATUS_OFFSET);
        dest_dev.num_bytes =
            RVM_ADD_LENGTH_TO_OFFSET(dest_dev.num_bytes, FILE_STATUS_OFFSET);
        status->log_start =
            RVM_MK_OFFSET(0, FILE_STATUS_OFFSET + LOG_DEV_STATUS_SIZE);
        status->log_head =
            RVM_SUB_LENGTH_FROM_OFFSET(status->log_head, RAW_STATUS_OFFSET);
        status->log_head =
            RVM_ADD_LENGTH_TO_OFFSET(status->log_head, FILE_STATUS_OFFSET);
        status->log_tail =
            RVM_SUB_LENGTH_FROM_OFFSET(status->log_tail, RAW_STATUS_OFFSET);
        status->log_tail =
            RVM_ADD_LENGTH_TO_OFFSET(status->log_tail, FILE_STATUS_OFFSET);
        if (!RVM_OFFSET_EQL_ZERO(status->prev_log_head)) {
            status->prev_log_head = RVM_SUB_LENGTH_FROM_OFFSET(
                status->prev_log_head, RAW_STATUS_OFFSET);
            status->prev_log_head = RVM_ADD_LENGTH_TO_OFFSET(
    ...
    while (!RVM_OFFSET_EQL(src_offset, log->dev.num_bytes)) {
        ...
        if (write_dev(&dest_dev, &dest_offset, log_buf->buf, log_buf->r_length,
                      NO_SYNCH) < 0) {
            pr_rvm_error(stderr, RVM_EIO, "writing log copy");
            goto err_exit;
        }
        src_offset  = RVM_ADD_LENGTH_TO_OFFSET(src_offset, log_buf->r_length);
        dest_offset = RVM_ADD_LENGTH_TO_OFFSET(dest_offset, log_buf->r_length);
    }
    goto exit;
err_exit:
...
static void do_commands(char *new_file /* new input file */)
{
    ...
    DO_FOREVER
    {
        ...
        if (read_prompt_line(stdout, "*", rvm_false) == NULL) {
            if (chk_sigint(NULL))
                stop_sw = rvm_false;
            else if (feof(in_stream) == 0) { /* i/o error */
